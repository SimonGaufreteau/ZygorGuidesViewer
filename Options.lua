...
function ZGV:Options_SetFromMode()
	self.db.profile.dispmode = self.db.profile.dispmodepri and self.db.profile.dispprimary
		or self.db.profile.dispsecondary
	for k, kv in pairs(self.db.profile.dispmode) do
		self.db.profile[k] = kv
	end
...
function ZGV:Options_DefineOptionTables()
	...
	local Setter_Loud = function(info, value)
		...
		INF = info
		ZGV:Print(
			info.option.name .. ": " .. (type(value) == "boolean" and (value and "|cff00ff88ON|r" or "|cffff0055OFF|r"))
				or "|cff00ff88" .. value .. "|r",
			nil,
			"FORCE"
		)
	end
	...
	local function AddOption(optname, optdata)
		...
		-- if optdata.type == "description" then
		-- 	optdata.font = optdata.font or ZGV.font_dialogsmall
		-- end
		-- if optdata.type == "toggle" then
		-- 	optdata.font = optdata.font or ZGV.font_dialog
		-- end
		-- if optdata.type == "execute" then
		-- 	optdata.font = optdata.font or ZGV.font_dialog
		-- end
		-- if optdata.type == "header" then
		-- 	optdata.font = optdata.font or ZGV.font_dialog
		-- end
		-- if optdata.type == "color" then
		-- 	optdata.font = optdata.font or ZGV.font_dialog
		-- end
		-- if optdata.type == "input" then
		-- 	optdata.font = optdata.font or ZGV.font_dialogsmall
		-- 	optdata.labelFont = optdata.labelFont or ZGV.font_dialogsmall
		-- 	optdata.buttonNormalFont = optdata.buttonNormalFont or ZGV.font_dialog
		-- end
		-- if optdata.type == "range" then
		-- 	optdata.labelFont = ZGV.font_dialog
		-- 	optdata.rangeFont = ZGV.font_dialogsmall
		-- 	optdata.valueFont = ZGV.font_dialogsmall
		-- end
		-- if optdata.type == "select" then
		-- 	--AddOption("", { type="description", name=optdata.name or optname, width="full", font=ZGV.font_dialog })
		-- 	--optdata.name=""
		-- 	optdata.labelFont = optdata.labelFont or ZGV.font_dialog
		-- 	optdata.valueFont = optdata.valueFont or ZGV.font_dialogsmall
		-- end
	...
	AddOptionGroup("cover", "Cover", "zygor", { name = L["name_plain"], desc = L["desc"] }) ---- OPTIONS: main
	do
		AddOption("show", {
			guiHidden = true,
			type = "execute",
		...
		AddOption("way", {
			...
			func = function(params)
				...
				if not input then
					ZGV:Print("Error parsing command.")
					return
				end
				if input == "" then
					ZGV:Print(
						"Set a destination waypoint, using travel directions.\n"
		...
		AddOption("debug", {
			...
			set = function()
				...
				ZGV:Print("Debugging: " .. (self.db.profile.debug and "|cff00ff88ON|r" or "|cffff0055OFF|r"))
			end,
		})
		...
		AddOption("debugflag", {
			...
			func = function(inp)
				...
				if inp == "" then
					for k, v in pairs(self.db.profile.debug_flags) do
						if type(v) == "table" then
							ZGV:Print("|c" .. (v.color or "ffffffff") .. k .. "|r " .. tostring(v.enabled))
						else
							ZGV:Print(k .. " " .. tostring(v))
						end
					end
					return
				end
				...
				if type(f) == "boolean" then
					f = { enabled = f }
					self.db.profile.debug_flags[inp] = f
				end
		...
		AddOption("detectpet", {
			...
			type = "execute",
			func = function()
				ZGV.CreatureDetector:Detect("force")
			end,
		})
		...
		AddOption("questid", {
			...
			func = function(params)
				...
				ZGV.QuestDB:SuggestGuidesForQuest(input)
			end,
		})
		...
		AddOption(
			"briefopentime",
			{ type = "range", min = 0.1, max = 2, step = 0.1, bigStep = 0.1, _default = 0.5, hidden = true }
		)
		--[[hidden--]]
		AddOption(
			"briefclosetime",
			{ type = "range", min = 0.1, max = 5, step = 0.1, bigStep = 0.1, _default = 1.0, hidden = true }
		)
		--[[hidden--]]
		...
		AddOption("share_masterslave", {
			...
			set = function(i, v)
				Setter_Simple(i, v)
				ZGV.Sync:Activate()
				if ZGV.CurrentGuide and ZGV.CurrentGuide.headerdata.shared then
					ZGV:ClearCurrentGuide()
				end
			end,
			disabled = function()
				return not self.db.profile.share_enabled
			end,
			hidden = true,
		})
		--[[hidden--]]
		AddOption("enable_vendor_tools", { type = "toggle", _default = true, width = "full", hidden = true })
		--[[hidden--]]
		AddOption("showgreyvalue", {
			type = "toggle",
			width = "full",
			set = function(i, v)
				Setter_Simple(i, v)
			...
			disabled = function()
				return not ZGV.db.profile.enable_vendor_tools
			end,
			_default = false,
			hidden = true,
		})
		--[[hidden--]]
		AddOption("autobuyframe", {
			type = "toggle",
			indent = 20,
			_default = true,
			width = "full",
			disabled = function()
				return not (self.db.profile.autobuy and ZGV.db.profile.enable_vendor_tools)
			end,
			indent = 15,
			width = "double",
			hidden = true,
		...
		AddOption("im_prefer_repair", {
			...
			hidden = true,
		}) -- Only find repair vendors
		--[[hidden--]]
		AddOption("autotrackquests", {
			type = "toggle",
			width = "full",
			_default = false,
			hidden = function()
				return not ZGV.db.profile.debug
			end,
		})
		--[[hidden--]]
		AddOption("audiocues", { type = "toggle", width = "full", _default = false, hidden = true })
		--[[hidden--]]
		AddOption("minimapzoom", {
			type = "toggle",
			width = "full",
			set = function(i, v)
				Setter_Simple(i, v)
				self.Pointer:MinimapZoomChanged()
			end,
			_default = false,
			hidden = true,
		})
		--[[hidden--]]
		AddOption("share_target", {
			hidden = true,
			type = "select",
			_default = "SAY",
			width = "single",
	...
	do
		...
		AddOption("hideexptracker", {
			type = "toggle",
			set = function(i, v)
				Setter_Simple(i, v)
				ZGV.Replacements:UpdateVisibility("force")
			end,
			_default = false,
		})
		AddOptionSep()
		...
		local style_sort_order_rev = {}
		for id, sort in pairs(style_sort_order) do
			style_sort_order_rev[sort] = id
		end
		...
		AddOption("skinstyle", {
			...
			hidden = function()
				return not self.CurrentSkin
					or not self.CurrentSkin.styles
					or not next(self.CurrentSkin.styles, next(self.CurrentSkin.styles))
			end,
			_default = "starlight",
		})
		...
		AddOption("opacity", {
			...
			disabled = function()
				return not ZGV.db.profile.opacitytoggle
			end,
			_default = 1,
			width = "single",
			_inline = true,
		...
		AddOption("framescale_s", {
			...
			get = function(i, v)
				for k, v in ipairs(framescales) do
					if self.db.profile.framescale == v then
						return k
					end
				end
				return 4
		...
		AddOption("fontsize_s", {
			...
			get = function(i, v)
				for k, v in ipairs(fontsizes) do
		...
		AddOption("widgetfloatscale_s", {
			...
			get = function(i, v)
				for k, v in ipairs(widgetfloatframescales) do
					if self.db.profile.widgetfloatscale == v then
						return k
					end
				end
				return 4
		...
		AddOption("resizeup", {
			...
			set = function(i, v)
				...
				if not v then -- right side up
					...
					ZGV.MasterFrame:SetPoint(
						"TOPLEFT",
						UIParent,
						"BOTTOMLEFT",
						ZGV.Frame:GetLeft() * ZGV.Frame:GetScale(),
						ZGV.Frame:GetTop() * ZGV.Frame:GetScale()
					)
				else -- upside-down
					ZGV.MasterFrame:ClearAllPoints()
					ZGV.MasterFrame:SetPoint(
						...
						"BOTTOMLEFT",
						ZGV.Frame:GetLeft() * ZGV.Frame:GetScale(),
						ZGV.Frame:GetBottom() * ZGV.Frame:GetScale()
					)
				end
				self:ReanchorFrame()
		...
		if ZGV.DEV then
			AddOption("zgvhideindungeons", {
				type = "toggle",
				name = "Hide viewer when entering dungeons and raids",
				width = "full",
				_default = false,
			})
			AddOption("showafterdungeons", {
				type = "toggle",
				name = "Show viewer again when leaving",
				indent = 20,
				width = "full",
				_default = true,
				disabled = function()
					return not self.db.profile.zgvhideindungeons
				end,
			})
		end
		AddOption("hideincombat", {
			...
			set = function(i, v)
				Setter_Simple(i, v)
				ZGV.Pointer:SetCombatHiding(v)
				ZGV.ActionBar:SetCombatHiding()
			end,
		})
		AddOption("hidebarincombat", {
			...
			disabled = function()
				return not self.db.profile.hideincombat
			end,
		})
	...
	do
		AddOption("showcountsteps", {
			type = "select",
			values = { --[[[0]=L["opt_showcountsteps_all"],--]]
				[1] = "1 (default)",
				[2] = "2",
				[3] = "3",
				[4] = "4",
		...
		if ZGV.IsRetail then
			AddOption("showallroles", {
				...
				width = "full",
				desc = function()
					return L["opt_showallroles_desc"]
						.. (
							UnitGroupRolesAssigned("Player") == "NONE"
								and "\n" .. L["opt_showallroles_descwarnnone"]
							or ""
						)
				end,
				_default = true,
			...
			AddOption(
				"showallroles",
				{ type = "toggle", width = "full", desc = L["opt_showallroles_desc"], _default = true }
			)
		end
		...
		AddOption("sync_snap", {
			...
			width = "full",
			disabled = function()
				return not ZGV.db.profile.sync_enabled
			end,
			set = function(i, v)
				Setter_Simple(i, v)
			end,
	...
	do
		...
		AddOption("autoacceptturnin", {
			type = "toggle",
			_default = not ZGV.DEV,
			width = "full",
			set = function(k, v)
				Setter_Simple(k, v)
				self.db.profile.autoaccept = v
				self.db.profile.autoturnin = v
			end,
		})
		AddOption("autoacceptturninall", {
			name = function()
				return L["opt_autoacceptturninall"]
			end,
			desc = function()
				return L["opt_autoacceptturninall_desc"]
			end,
			type = "toggle",
			width = "single",
			disabled = function()
		...
		AddOption(
			"",
			{ type = "description", name = L["opt_header_automation_inventory"], font = ZGV.font_dialog_gray }
		)
		AddOption("autobuy", {
			type = "toggle",
			_default = true,
		...
		AddOption("showgreysellbutton", {
			...
			set = function(i, v)
				Setter_Simple(i, v)
				ZGV.Inventory:SetUpGreySellButton()
				ZGV.Inventory.greysellbutton:SetShown(v)
			end,
			disabled = function()
				return not ZGV.db.profile.enable_vendor_tools
			end,
		...
		AddOption("", {
			...
			hidden = function()
				return IsInGuild() or ZGV.db.profile.autorepair <= 2
			end,
		})
		...
		AddOption("", {
			...
			font = ZGV.font_dialog_red,
			hidden = function()
				return not IsInGuild() or CanGuildBankRepair() or ZGV.db.profile.autorepair <= 2
			end,
		})
	...
	do
		AddOption("enable_actionbuttons", {
			type = "toggle",
			get = function()
		...
		AddOption("enable_actionbar", {
			...
			width = "SINGLE",
			disabled = function()
				return not self.db.profile.enable_actionbuttons
			end,
		})
		...
		AddOption("actionbar_scale_s", {
			...
			get = function(i, v)
				for k, v in ipairs(framescales) do
					if self.db.profile.actionbar_scale == v then
						return k
					end
				end
				return 4
	...
	do
		AddOption("arrowshow", {
			...
			set = function(i, v)
				Setter_Simple(i, v)
				self.Pointer:UpdateArrowVisibility()
			end,
			_default = true,
		})
		...
		AddOption("arrowscale_s", {
			...
			get = function(i, v)
				for k, v in ipairs(arrowscales) do
					if self.db.profile.arrowscale == v then
						return k
					end
				end
				return 2
		...
		AddOption("arrowfontsize_s", {
			...
			get = function(i, v)
				for k, v in ipairs(arrowfontsizes) do
					if self.db.profile.arrowfontsize == v then
						return k
					end
				end
				return 4
		...
		AddOptionSpace()
		AddOption("arrowoutline", {
			type = "toggle",
			width = "double",
			_default = false,
			set = function(i, v)
				Setter_Simple(i, v)
				ZGV.Pointer:SetFontSize(ZGV.db.profile.arrowfontsize)
			end,
		})
		...
		AddOption("arrowskinselect", {
			...
			get = function()
				for id, skin in pairs(self.Pointer.ArrowSkins) do
					if skin.name == ZGV.db.profile.arrowskin then
						return skin.order
					end
				end
				return 2
		...
		AddOption("pathfinding", {
			hidden = function()
				return LibRover.is_stub
			end, -- and not ZGV.guidesets['PetsMountsA'] and not ZGV.guidesets['PetsMountsH'],
			type = "toggle",
			set = function(i, v)
				Setter_Simple(i, v)
		...
		for index = 1, 4 do
			AddOption("travelmode_" .. index, {
				type = "toggle",
				set = function(i, v)
					if self.db.global.travelmode == index then
						return
					end -- we are faking radio, so can't uncheck oneself
					--for j=1,5 do
				...
				get = function()
					return self.db.global.travelmode == index
				end,
				width = "double",
				_default = j == 3,
			})
		...
		AddSubgroup("travelmode_custom", {
			...
			inline = true,
			hidden = function()
				return ZGV.db.global.travelmode ~= 4
			end,
		})
	...
	do
		...
		if ZGV.IsRetail or ZGV.IsClassicMOP then
			AddOptionSpace()
			AddOption("poienabled", {
				type = "toggle",
				width = "double",
				set = function(i, v)
					Setter_Simple(i, v)
					ZGV.Poi:ChangeState(v)
				end,
				_default = true,
			})
			AddOptionSep()
			AddOption("poisize", {
				...
				style = "slider",
				values = {
					[17] = L["opt_framescale_s_small"],
					[20] = L["opt_framescale_s_normal"],
					[23] = L["opt_framescale_s_large"],
				},
				set = function(i, v)
					Setter_Simple(i, v)
					ZGV.Pointer:RefreshDynamicValues()
				end,
				_default = 20,
				disabled = function()
					return not self.db.profile.poienabled
				end,
				width = "single",
			})
			AddOption("poialphatoggle", {
				...
				set = function(i, v)
					Setter_Simple(i, v)
					ZGV.db.profile.poialpha = (v and 0.68 or 1)
					ZGV.Pointer:RefreshDynamicValues()
				end,
				_default = false,
				disabled = function()
			...
			AddOption("poishow_rare", {
				...
				set = function(i, v)
					Setter_Simple(i, v)
					ZGV.db.profile.hideguide.rare = not v
					ZGV.Poi:ChangeState(true)
				end,
				get = function()
					return not ZGV.db.profile.hideguide.rare
				end,
				disabled = function()
					return not self.db.profile.poienabled
				end,
			...
			AddOption("poishow_treasure", {
				...
				set = function(i, v)
					Setter_Simple(i, v)
					ZGV.db.profile.hideguide.treasure = not v
					ZGV.Poi:ChangeState(true)
				end,
				get = function()
					return not ZGV.db.profile.hideguide.treasure
				end,
				disabled = function()
					return not self.db.profile.poienabled
				end,
			...
			AddOption("worldquestlocal", {
				type = "toggle",
				_default = true,
				width = "full",
				desc = L["opt_worldquestlocal_desc"],
				disabled = function()
					return not self.db.profile.worldquestenable
				end,
				indent = 20,
			})
			AddOption("worldquestmap", {
				type = "toggle",
				width = "full",
				_default = true,
				set = function(i, v)
					Setter_Simple(i, v)
					if not v then
						self.db.profile.n_popup_wq = false
					end
				end,
			})
		end
		...
		AddOption("preview_duration", {
			type = "select",
			values = {
				[0] = L["opt_preview_duration_perm"],
				[3] = L["opt_preview_duration_3"],
				[5] = L["opt_preview_duration_5"],
				[10] = L["opt_preview_duration_10"],
			},
			set = function(i, v)
				Setter_Simple(i, v)
				ZGV.PointerMap:UpdateSettings()
	...
	do
		...
		AddOption("upgradebest", {
			...
			_inline = true,
			disabled = function()
				return not (self.db.profile.autogear and self.db.profile.markupgrades)
			end,
		})
		...
		AddOption("upgradeslot", {
			...
			set = function(i, v)
				...
				if v > 0 then
					...
					if slot_ids[v] == INVSLOT_FINGER1 then
						print("ring")
						ZGV.db.profile.upgradeslot2 = INVSLOT_FINGER2
					end
					if slot_ids[v] == INVSLOT_TRINKET1 then
						print("trink")
						ZGV.db.profile.upgradeslot2 = INVSLOT_TRINKET2
					end
				else
			...
			get = function(i, v)
				for i, slot in ipairs(slot_ids) do
					if slot == ZGV.db.profile.upgradeslot then
						return i
					end
				end
				return 0
		...
		AddOption("autoselectitem", {
			type = "toggle",
			width = "double",
			_default = false,
			disabled = function()
				return not self.db.profile.questitemselector
			end,
			indent = 20,
		})
		AddOption("autogearauto", {
			type = "toggle",
			width = "full",
			_default = false,
			disabled = function()
		...
		if ZGV.IsRetail or ZGV.IsClassicWOTLK or ZGV.IsClassicCATA or ZGV.IsClassicMOP then
			AddOption("autogear_keepheirlooms", {
				type = "toggle",
				width = "double",
				_default = false,
				disabled = function()
		...
		--[[hidden--]]
		AddOption("itemscore_tooltips_azerite", {
			type = "toggle",
			width = "full",
			_default = true,
			indent = 20,
			set = Setter_Simple,
			disabled = function()
				return not (self.db.profile.autogear and self.db.profile.itemscore_tooltips)
			end,
			hidden = true,
		})
		...
		local modifierkeys =
			{ { "IsShiftKeyDown", "Shift" }, { "IsControlKeyDown", "Control" }, { "IsAltKeyDown", "Alt" } }
		...
		AddOption("gearreportkey", {
			...
			get = function(i, v)
				for k, v in ipairs(modifierkeys) do
					if self.db.profile.gearreportkey == v[1] then
						return k
					end
				end
			end,
			...
			disabled = function()
				return not (
					self.db.profile.autogear
					and self.db.profile.markupgrades
					and self.db.profile.badupgradehotkey
				)
			end,
		})
		...
		AddOption("gearreportmouse", {
			...
			get = function(i, v)
				for k, v in ipairs(mousekeys) do
					if self.db.profile.gearreportmouse == v[1] then
						return k
					end
				end
			end,
		...
		AddOption("", {
			type = "description",
			name = L["opt_gear_sources_dungeons"]:format(),
			font = ZGV.font_dialog_gray,
			width = "full",
			disabled = function()
				return not self.db.profile.autogear_finder
			end,
		})
		AddOption("gear_1", {
		...
		if ZGV.IsRetail then
			...
			AddOption("gear_8_level", {
				...
				width = 400,
				hidden = function()
					return not self.db.profile.gear_8
				end,
				set = function(i, v)
					Setter_Simple(i, v)
					ZGV.ItemScore.GearFinder:ClearResults()
				...
				disabled = function()
					return not (self.db.profile.autogear_finder and self.db.profile.gear_8)
				end,
			})
		end
		...
		AddOption("", {
			type = "description",
			name = L["opt_gear_sources_raids"]:format(),
			font = ZGV.font_dialog_gray,
			width = "full",
		})
		if ZGV.IsRetail then
			...
			AddOption("gear_14", {
				...
				set = function(i, v)
					Setter_Simple(i, v)
					ZGV.db.profile.gear_3 = v
					ZGV.db.profile.gear_4 = v
					ZGV.ItemScore.GearFinder:ClearResults()
				end,
				disabled = function()
					return not self.db.profile.autogear_finder
				end,
			}) -- also setting filters for prelfr raids
			AddOption("gear_15", {
				name = PLAYER_DIFFICULTY2,
				type = "toggle",
				width = "100",
				...
				set = function(i, v)
					Setter_Simple(i, v)
					ZGV.db.profile.gear_5 = v
					ZGV.db.profile.gear_6 = v
					ZGV.ItemScore.GearFinder:ClearResults()
				end,
				disabled = function()
					return not self.db.profile.autogear_finder
	...
	do
		...
		if ZGV.IsRetail then
			...
			AddOption("gear_selected_spec", {
				...
				values = function()
					...
					for specnum, specdata in pairs(ZGV.ItemScore.Defaults[tag]) do
						if
							specnum < 5
							or (ZGV.ItemScore.playerlevel < (ZGV.db.char.gear_selected_class == 13 and 60 or 10))
						then -- show starter spec only on low level characters
				...
				set = function(i, v)
					ZGV.db.char.gear_selected_spec = v
					Setter_Simple(i, v)
				end,
				get = function()
					return ZGV.db.char.gear_selected_spec
				end,
				_inline = true,
			})
			...
			AddOption("gear_selected_build", {
				...
				name = "",
				values = function()
					return ZGV.ItemScore.Builds[playerclass]
				end,
				get = function()
					return ZGV.db.char.gear_selected_build
				end,
				set = function(i, v)
					Setter_Simple(i, v)
		...
		local function display_stat(class, spec, name)
			...
			if ZGV.db.profile.gearshowallstats then
				return true
			end
			...
			if profile[prefix .. name] == "" or profile[prefix .. name] == "0" then
				return false
			end

			-- user set the stat, do show
			if profile[prefix .. name] and tonumber(profile[prefix .. name]) > 0 then
				return true
			end

			-- zygor default exists, do show
			if statset[name] then
				return true
			end
		...
		local function get_stat(class, spec, name)
			...
			if
				profile[prefix .. name]
				and tonumber(profile[prefix .. name])
				and tonumber(profile[prefix .. name]) > 0
			then
				return tostring(profile[prefix .. name])
			end

			-- zygor default exists, do show
			if statset[name] then
				return tostring(statset[name])
			end
		...
		for specnum, specdata in pairs(ZGV.ItemScore.Defaults[playerclass]) do
			...
			AddOption("", {
				...
				width = 150,
				hidden = function()
					return not ZGV.ItemScore:UsesCustomWeights(playerclass, specnum)
				end,
			})
			AddOptionSep()
			...
			for index = 1, #ZGV.ItemScore.Keywords do
				...
				if not stat.multi then
					AddOption("", {
						...
						width = 150,
						hidden = function()
							return not display_stat(playerclass, specnum, stat.blizz)
						end,
					})
					AddOption(groupname .. "_" .. stat.blizz, {
						...
						width = 130,
						get = function()
							return get_stat(playerclass, specnum, stat.blizz)
						end,
						set = function(i, v)
	...
	do
		...
		if ZGV.Gold.Appraiser then
			...
			AddOption("autoscan", {
				...
				width = "full",
				disabled = function()
					return not ZGV.db.profile.auction_enable
				end,
			})
			AddOption("quickscan", {
				type = "toggle",
			...
			--[[hidden--]]
			AddOption("auction_autoshow_tab", {
				type = "toggle",
				_default = false,
				width = "double",
				disabled = function()
			...
			AddOption("gold_tooltips_out", {
				...
				pulloutWidth = "single",
				disabled = function()
					return not ZGV.db.profile.gold_tooltips_show
				end,
			})
			--[[
				AddOptionSep()
	...
	do
		...
		AddOption("", {
			...
			name = L["opt_nc_types"],
			hidden = function()
				return not self.db.profile.nc_showall
			end,
			font = ZGV.font_dialog,
		})
		AddOptionSep()
		AddOption("nc_welcome", {
			type = "toggle",
			width = "full",
			_default = true,
			hidden = function()
		...
		AddOption("nc_gear", {
			...
			disabled = function()
				return not self.db.profile.nc_enable or not self.db.profile.autogear
			end,
		})
		AddOption("nc_gear_max", {
			type = "toggle",
			width = "full",
			_default = false,
			indent = 20,
			...
			disabled = function()
				return not self.db.profile.nc_enable
					or not self.db.profile.nc_gear
					or not self.db.profile.autogear
					or self.db.profile.autogear_max
			end,
		})
		...
		if ZGV.IsRetail then
			AddOption("nc_monk", {
				...
				disabled = function()
					return not (select(3, UnitClass("player")) == 10 and self.db.profile.nc_enable)
				end,
			})
		end
		if not ZGV.IsClassic then
			AddOption("nc_dailyquest", {
				type = "toggle",
				width = "full",
				_default = true,
				hidden = function()
		...
		if ZGV.IsClassic or ZGV.IsClassicTBC or ZGV.IsClassicWOTLK then
			...
			AddOption("", {
				...
				disabled = function()
					return not (self.db.profile.nc_enable and self.db.profile.nc_skills)
				end,
			})
			AddOptionSep()
			AddOption("", {
				type = "description",
				name = L["opt_group_notify"],
				font = ZGV.font_dialog,
				indent = 20,
				hidden = function()
					return not self.db.profile.nc_showall
			...
			AddOption("nc_skills_optional", {
				type = "toggle",
				width = "single",
				indent = 20,
				set = function(i, v)
					Setter_Simple(i, v)
					ZGV.Skills:RefreshSkillPopup()
				end,
				_default = true,
				hidden = function()
					return not self.db.profile.nc_showall
			...
			AddOption("", {
				type = "description",
				name = L["opt_nc_skillstriggertitle"],
				hidden = function()
					return not self.db.profile.nc_showall
				end,
				font = ZGV.font_dialog,
			})
			AddOption("nc_skills_login", {
				type = "toggle",
				indent = 20,
				width = "double",
				_default = true,
	...
	do
		...
		if not ZGV.IsRetail then
			AddOption("showbagspace", {
				type = "toggle",
				width = "full",
				_default = false,
				set = function(i, v)
					Setter_Simple(i, v)
					ZGV.Inventory:UpdateBagspaceText()
				end,
			})
		end
			...
			set = function(i, v)
				...
				if ZGV.BugReport.GuideRating.GuideRatingViewer or ZGV.BugReport.GuideRating.NoRatingFrame then
					...
					then
						...
						ZGV.BugReport.GuideRating:ShowGuideRating()
						ZGV.BugReport.GuideRating.GuideRatingViewer:Hide()
						ZGV.BugReport.GuideRating:ShowGuideRating()
					end
				else
					ZGV.BugReport.GuideRating:ShowGuideRating()
		...
		AddOption("spam_levelup_emote", {
			...
			width = "double",
			disabled = function()
				return not self.db.profile.spam_levelup
			end,
			indent = 20,
			_default = true,
		})
	...
	do
		...
		tinsert(ZGV.startups, {
			"Options overwrite wtf?",
			function()
				ZGV.confirmOverwritePopup = ZGV.PopupHandler:NewPopup("ConfirmOverwrite", "default")
			end,
		})
		...
		AddOption("profile_reset", {
			...
			func = function(i, value)
				...
				--ZGV.db:ResetProfile()
				local reset_tables =
					{ "menu", "display", "navi", "poi", "notification", "gear", "itemscore", "gold", "enhancements" }
		...
		AddOption("", {
			type = "description",
			name = L["opt_profile_manage"]:format(),
			font = ZGV.font_dialog_gray,
			width = "double",
		})
		AddOptionSep()
		...
		AddOption("wipe_settings", {
			desc = function()
				return ZGV.db.profile.temp_tried_to_wipe and L["opt_wipe_settings_desc2"] or L["opt_wipe_settings_desc"]
			end,
			type = "execute",
			func = function(i, value)
				if IsShiftKeyDown() then
					...
					C_Timer.After(2, function()
						ZGV.db.profile.temp_tried_to_wipe = nil
					end)
					...
					for i, v in ipairs(chi) do
						if v.userdata[1] == "wipe_settings" then
		...
		AddOption(
			"",
			{ type = "description", name = L["opt_widgetsbackup"], font = ZGV.font_dialog_gray, width = "full" }
		)
		AddOptionSep()
	...
	do
		...
		AddOption(
			"",
			{ type = "description", name = L["opt_about_desc4"]:format(self.version), font = ZGV.font_dialog }
		)
		AddOptionSpace()
		AddOption("", { type = "description", name = L["opt_tech_support_header"], font = ZGV.font_dialog_gray })
		AddOption(
			"",
			{ type = "description", name = L["opt_tech_support"]:format(self.version), font = ZGV.font_dialog }
		)
		AddOptionSep()
		AddOption("report", {
			type = "execute",
	...
	AddOptionGroup(
		...
		"DebugSet",
		"zgdebugset",
		{ name = "Debug: settings", guiHidden = not self.DEV or self.db.profile.hide_dev_once }
	)
	do
		AddOption("loadguidesfully", {
			name = "Load full guides at startup",
			desc = "Horribly increases startup time, but loads and checks all guides.\nRestart for this to take effect.",
			type = "toggle",
			width = "full",
		})
		...
		AddOption("fpsgraph", {
			name = "FPS Graph",
			desc = "Show a detailed FPS graph. Max=60fps.",
			type = "toggle",
			width = "full",
			_default = false,
			set = function(i, v)
		...
		AddOption("debug_display", {
			name = "Show Zygor developers information",
			type = "toggle",
			width = "full",
			_default = false,
			set = function(i, v)
		...
		AddOption("disabledev", {
			name = "Disable dev menu for next reload",
			type = "execute",
			width = "double",
			func = function()
				self.db.profile.hide_dev_once = true
				ReloadUI()
			end,
		})
	end
	...
	AddOptionGroup(
		...
		"DebugFake",
		"zgdebugfake",
		{ name = "Debug: faking stuff", guiHidden = not self.DEV or self.db.profile.hide_dev_once }
	)
	do
		AddOption("fakelevel", {
		...
		AddOption("fakecombat", {
			...
			set = function(i, v)
				Setter_Simple(i, v)
				if (v and not UnitAffectingCombat("player")) or (not v and UnitAffectingCombat("player")) then
		...
		if ZGV.IsClassic then
			AddOption("fakeHC", {
				...
				set = function(i, v)
					Setter_Simple(i, v)
					if ZGV.db.profile.fakeHC == 0 then
						ZGV.db.char.fakehardcore = true
					end
					if ZGV.db.profile.fakeHC == 1 then
						ZGV.db.char.fakehardcore = false
					end
					if ZGV.db.profile.fakeHC == 2 then
						ZGV.db.char.fakehardcore = nil
					end
				end,
				_default = 2,
			})
		...
		do -- Fake skills
			...
			if ZGV.IsClassic or ZGV.IsClassicTBC or ZGV.IsClassicWOTLK or ZGV.IsClassicCATA or ZGV.IsClassicMOP then
				...
				AddOption("fakeskilllevel", {
					...
					disabled = function()
						return not ZGV.db.profile.fakeskillcheck
					end,
					width = "half",
					_default = 0,
				})
				...
				AddOption("fakeskilllist", {
					name = function()
						...
						if not next(ZGV.db.profile.fakeskills) then
							s = s .. "  none"
						end
						return s
					end,
				...
				AddOption("fakeskillmax", {
					...
					values = function()
						...
						elseif
							fakeskill:find("Outland")
							or fakeskill:find("Northrend")
							or fakeskill:find("Cataclysm")
							or fakeskill:find("Pandaren")
							or fakeskill:find("Way of")
						then
		...
		do -- Fake Reputation
			AddOption("fakerep", {
				...
				type = "select",
				values = function()
					return self.factions_mentioned or {}
				end,
				set = function(i, v)
					Setter_Simple(i, v)
			...
			AddOption("fakerepstanding", {
				...
				type = "select",
				values = function()
					local standings = ZGV:GetReputation(ZGV.db.profile.fakerep).reptypemeta.standings
					local ret = {}
					for i, v in ipairs(standings) do
						ret[i] = v.name
					end
					return ret
				end,
				...
				disabled = function()
					return not ZGV.db.profile.fakerepcheck
				end,
				_default = 4,
				_inline = true,
			})
			AddOption("fakereplist", {
				name = function()
					...
					if not next(ZGV.db.profile.fakereps) then
						s = s .. "  none"
					end
					return s
				end,
		...
		do -- Travel fakes
			...
			AddOptionSep()
			for i, n in ipairs({
				{ "flightazeroth", "Azeroth" },
				{ "flightnorthrend", "Northrend" },
				{ "flightpandaria", "Pandaria" },
				{ "flightdraenor", "Draenor" },
				{ "flightlegion", "Legion" },
				{ "flightbfa", "BfA" },
				{ "flightsl", "Shadowlands" },
				{ "flightdf", "Dragonflight" },
				{ "dragonriding", "Dragonriding with skills" },
			}) do --EXPANSION
		...
		do -- Fake Popups
			...
			AddOption("faketoast_1", {
				...
				func = function(info, val)
					ZGV.NotificationCenter:AddEntry(
						"welcome",
						"Welcome to Zygor Guides",
						"This is a test welcome message",
						{ cleartype = true, keeponclick = true }
					)
				end,
			})
			AddOption("faketoast_2", {
				type = "execute",
				width = 160,
				name = "Weekly Quests",
				func = function(info, val)
					ZGV.NotificationCenter:AddEntry(
						"weekly",
						"Weekly Quests have reset on the server.",
						"",
						{ dontsave = true }
					)
				end,
			})
			AddOption("faketoast_2.5", {
				type = "execute",
				width = 160,
				name = "Daily Quests",
				func = function(info, val)
					ZGV.NotificationCenter:AddEntry(
						"daily",
						"Daily Quests have reset on the server.",
						"",
						{ dontsave = true }
					)
				end,
			})
			AddOption("faketoast_3", {
				type = "execute",
				width = 160,
				name = "Events",
				func = function(info, val)
					ZGV.NotificationCenter:AddEntry(
						"event",
						"The Noblegarden Event is in progress.",
						"Click to see guides for it.",
						{ folder = "EVENTS\\Noblegarden" }
					)
				end,
			})
			AddOption("faketoast_4", {
				type = "execute",
				width = 160,
				...
				func = function(info, val)
					ZGV.NotificationCenter:AddEntry(
						"orientation",
						"Run Startup Orientation to improve guide directions",
						"Click to run now",
						{ cleartype = true }
					)
				end,
			})
			AddOption("faketoast_5", {
				type = "execute",
				width = 160,
				name = "Guide Updates",
				func = function(info, val)
					ZGV.NotificationCenter:AddEntry(
						"guides",
						"See new and updated guides",
						"",
						{ cleartype = true, keeponclick = true }
					)
				end,
			})
			AddOption("faketoast_6", {
				type = "execute",
				width = 160,
			...
			AddOption("fakepopup_11", {
				type = "execute",
				width = 140,
				name = "Boosted",
				func = function(info, val)
					ZGV:IsBoosted(100, true)
				end,
			})
			--[[
				...
				--]]
			AddOption("fakepopup_6", {
				type = "execute",
				width = 140,
				name = "Suggest dungeon",
				func = function(info, val)
					ZGV.GuideFuncs:SuggestDungeonGuide(
						ZGV:GetGuideByTitle("Zygor's Dungeon Guides\\Classic Dungeons\\The Stormwind Stockade")
					)
				end,
			})
			AddOption("fakepopup_6.5", {
				type = "execute",
				width = 185,
				name = "Suggest dungeon (multi)",
				func = function(info, val)
					ZGV.GuideFuncs:SuggestDungeonGuide(
						ZGV:GetGuideByTitle("Dungeon Guides\\Draenor Raids\\Highmaul - Normal/Heroic")
					)
				end,
			})
			AddOptionSep()
			--[[
				...
				AddOptionSep()
				--]]
			AddOption("fakepopup_3", {
				type = "execute",
				width = 140,
				name = "Riding training",
				func = function(info, val)
					ZGV.GuideFuncs:LearnMountGuideSuggestion(ZGV.FakeLevelForRiding or 20)
				end,
			})
			AddOption("fakepopup_3_dropdown", {
				...
				set = function(i, v)
					ZGV.FakeLevelForRiding = v
				end,
				get = function()
					return ZGV.FakeLevelForRiding or 20
				end,
				_inline = true,
			})
			AddOptionSep()

			AddOption("fakepopup_4", {
				type = "execute",
				width = 140,
				name = "Monk quests",
				func = function(info, val)
					ZGV.GuideFuncs:MonkQuest(ZGV.FakeLevelForMonk or 1)
				end,
			})
			AddOption("fakepopup_4_dropdown", {
				...
				set = function(i, v)
					ZGV.FakeLevelForMonk = v
				end,
				get = function()
					return ZGV.FakeLevelForMonk or 1
				end,
				_inline = true,
			})
			AddOptionSep()
			--[[
				...
				--]]
			AddOption("fakepopup_8", {
				type = "execute",
				width = 140,
				name = "Bad guide popup",
				func = function(info, val)
					ZGV:SetGuide(ZGV.CurrentGuide, nil, "forcebad")
				end,
			})
			--[[
			...
			AddOption("faketoast_11", {
				...
				func = function(info, val)
					ZGV.NotificationCenter:AddEntry(
						...
						"Baby Blizzard Bear",
						"",
						{ guide = "PETSMOUNTS\\Battle Pets\\Beast Pets\\Baby Blizzard Bear" }
					)
				end,
			})
			AddOption("faketoast_12", {
				type = "execute",
				width = 160,
		...
		do -- Fake goal status
			...
			AddOption("fakegoal_list", {
				...
				get = function()
					return ZGV.FakeStepNum or 1
				end,
				_inline = true,
			})

			local goal_status =
				{ "--default--", "hidden", "passive", "complete", "warning", "impossible", "incomplete" }
			AddOption("fakegoal_list", {
				name = "Status",
				type = "select",
				values = goal_status,
				set = function(i, v)
					if not ZGV.CurrentStep or not ZGV.CurrentStep.goals[1] then
						return
					end
		...
		if ZGV.IsRetail then
			...
			AddOption("fake_garrison", {
				...
				values = function()
					return {
						[-1] = "--not set: " .. ZGV.GetGarrisonLevel(true) .. "--",
						[0] = "not built",
						[1] = "level 1",
						[2] = "level 2",
						[3] = "level 3",
			...
			AddOption("fake_covenant", {
				...
				values = function()
					local val = {
						[-1] = "--not set: " .. (C_Covenants.GetCovenantData(C_Covenants.GetActiveCovenantID()) or {
							textureKit = "none",
						}).textureKit .. "--",
						[0] = "None",
					}
			...
			AddOption("fake_covenant_feature_transport", {
				...
				values = function()
					local val = {
						[-1] = "--not set: " .. ZGV.Parser.ConditionEnv.covenantnetwork() .. "--",
						[0] = 0,
						[1] = 1,
						[2] = 2,
						[3] = 3,
	...
	AddOptionGroup(
		...
		"DebugDig",
		"zgdebugdig",
		{ name = "Debug: data digging", guiHidden = not self.DEV or self.db.profile.hide_dev_once }
	)
	do
		if ZGV.IsRetail then
			AddOption("sep00scen", { type = "header", name = "Scenarios / Dungeons" })
			AddOption("dumpscenario", {
				name = "Dump scenario objectives",
				width = "double",
				disabled = function()
					return not C_Scenario.IsInScenario()
				end,
				desc = "",
				type = "execute",
				func = function()
		...
		AddOption("foglightdumpnew", {
			name = "Update all maps data",
			type = "execute",
			width = "double",
			func = function()
				ZGV.Testing.FoglightDumper:UpdateAll()
			end,
		})
		AddOption("foglightdumpnew", {
			name = "Update current zone data",
			desc = "Map visible in map frame, or one player is on",
			type = "execute",
			width = "double",
			func = function()
				ZGV.Testing.FoglightDumper:UpdateCurrent()
			end,
		})
		AddOption("foglightdumpnew", {
			name = "Refresh current zone data",
			desc = "Map visible in map frame, or one player is on.\nGet fresh copy of overlays, ignoring current data",
			type = "execute",
			width = "double",
			func = function()
				ZGV.Testing.FoglightDumper:RefreshCurrent()
			end,
		})

		AddOption("sep00score", { type = "header", name = "Itemscore" })
		AddOption("score_this", {
			type = "execute",
			name = "Score cursor item",
			desc = function()
				if GetCursorInfo() == "item" then
					return ZGV.ItemScore:ScoreCursor("quiet") .. "\n\nClick button to send this to a Dump window."
				else
					return "Drag an item here to score it..."
				end
			end,
			func = function(info, val)
		...
		if ZGV.Testing and ZGV.Testing.LootDumper then
			...
			AddOption("lootdumper_abort", {
				...
				width = 140,
				disabled = function()
					return not LootDumper.Events
				end,
				func = function(info, val)
					LootDumper.Worker = nil
				end,
			})
			...
			AddOption("lootdumper_getall", {
				...
				width = 140,
				disabled = function()
					return not LootDumper.Events or LootDumper.GotAllInstancesInfo
				end,
				func = function(info, val)
					LootDumper:GetAllInstancesInfo()
				end,
			})
			...
			AddOption("lootdumper_select_instance", {
				...
				name = "",
				values = function()
					return LootDumper.OptionsMap or {}
				end,
				disabled = function()
					return not LootDumper.Events
				end,
			...
			AddOption("lootdumper_loot_current", {
				...
				width = 180,
				disabled = function()
					return not LootDumper.SelectedInstance
				end,
				func = function(info, val)
					LootDumper:GetInstanceLoot(LootDumper.SelectedInstance)
				end,
			})
			...
			AddOption("lootdumper_generate_current", {
				...
				disabled = function()
					return not (
						LootDumper.Events
						and LootDumper.SelectedInstance
						and LootDumper.Instances[LootDumper.SelectedInstance].GatheredLoot
					)
				end,
				func = function(info, val)
			...
			AddOption("lootdumper_loot_all", {
				...
				func = function(info, val)
					LootDumper:GetAllInstancesLoot()
				end,
			})
			...
			AddOption("lootdumper_generate_all", {
				...
				name = function()
					local gathered = 0
					if LootDumper.Instances then
						for instnum, instdata in pairs(LootDumper.Instances) do
							if instdata.GatheredLoot then
								gathered = gathered + 1
							end
						end
					end
				...
				func = function(info, val)
					LootDumper:GetAllInstancesGuides()
				end,
			})
			...
			AddOption("lootdumper_dump_info", {
				...
				func = function(info, val)
					Spoo(LootDumper.InstancesByName)
				end,
			})
			...
			AddOption("lootdumper_dump_lfg", {
				...
				width = 280,
				disabled = function()
					return not LootDumper.GotAllInstancesInfo
				end,
				func = function(info, val)
					LootDumper:DumpLFGData()
				end,
			})
	...
	AddOptionGroup(
		...
		"DebugRemoved",
		"zgdebugremoved",
		{ name = "Debug: removed options", guiHidden = not self.DEV or self.db.profile.hide_dev_once }
	)
	do
		AddOption("n_popup_guides", {
			type = "toggle",
			width = "full",
			_default = true,
			disabled = function()
				return not self.db.profile.n_popup_enable
			end,
		}) --This should not be needed since next guide popups went away with Guide Reviews, but it's everywhere and bugs out the floating toast if removed

		AddOption("guide_viewer_advanced", { type = "toggle", width = "double", plusminus = true })
		AddSubgroup("advancedcust_subgroup", {
			hidden = function()
				return not self.db.profile.guide_viewer_advanced
			end,
		})

		AddOption("", {
			type = "header",
			name = L["opt_step_display_header"]:format(),
			hidden = function()
				return not self.db.profile.guide_viewer_advanced
			end,
		})
		...
		AddOption("", {
			type = "header",
			name = L["opt_goalbackcolor_desc"],
			hidden = function()
				return not self.db.profile.guide_viewer_advanced
			end,
		})

		AddOption("goalbackgrounds", {
			type = "toggle",
			_default = true,
			hidden = function()
				return not self.db.profile.guide_viewer_advanced
		...
		AddOption("goalbackprogress", {
			type = "toggle",
			disabled = function()
				return not self.db.profile.goalbackgrounds
			end,
		...
		AddOption("", {
			type = "description",
			width = "double",
			name = L["opt_goalcolor_completion_desc"],
			hidden = function()
				return not self.db.profile.guide_viewer_advanced
			end,
		})
		AddOption("", {
			type = "description",
			width = "single",
			name = L["opt_goalcolor_other_desc"],
			hidden = function()
				return not self.db.profile.guide_viewer_advanced
			end,
		})
		...
		AddOption("goalbackincomplete", {
			...
			get = function()
				return rgb2list(self.db.profile.goalbackincomplete)
			end,
			set = function(_, r, g, b, a)
				self.db.profile.goalbackincomplete = { r = r, g = g, b = b, a = a }
				self:UpdateFrame()
			end,
			hasAlpha = true,
			hidden = function()
		...
		AddOption("goalbackimpossible", {
			...
			get = function()
				return rgb2list(self.db.profile.goalbackimpossible)
			end,
			set = function(_, r, g, b, a)
				self.db.profile.goalbackimpossible = { r = r, g = g, b = b, a = a }
				self:UpdateFrame()
			end,
			hasAlpha = true,
			hidden = function()
		...
		AddOption("goalbackprogressing", {
			...
			width = "double",
			disabled = function()
				return not self.db.profile.goalbackgrounds or not self.db.profile.goalbackprogress
			end,
			get = function()
				return rgb2list(self.db.profile.goalbackprogressing)
			end,
			set = function(_, r, g, b, a)
				self.db.profile.goalbackprogressing = { r = r, g = g, b = b, a = a }
				self:UpdateFrame()
			end,
			hasAlpha = true,
			hidden = function()
		...
		AddOption("goalbackwarning", {
			...
			get = function()
				return rgb2list(self.db.profile.goalbackwarning)
			end,
			set = function(_, r, g, b, a)
				self.db.profile.goalbackwarning = { r = r, g = g, b = b, a = a }
				self:UpdateFrame()
			end,
			hasAlpha = true,
			hidden = function()
		...
		AddOption("goalbackcomplete", {
			...
			get = function()
				return rgb2list(self.db.profile.goalbackcomplete)
			end,
			set = function(_, r, g, b, a)
				self.db.profile.goalbackcomplete = { r = r, g = g, b = b, a = a }
				self:UpdateFrame()
			end,
			hasAlpha = true,
			hidden = function()
		...
		AddOption("goalbackaux", {
			...
			get = function()
				return self.db.profile.goalbackaux.r,
					self.db.profile.goalbackaux.g,
					self.db.profile.goalbackaux.b,
					self.db.profile.goalbackaux.a
			end,
			set = function(_, r, g, b, a)
				self.db.profile.goalbackaux = { ["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a }
				self:UpdateFrame()
			end,
			hasAlpha = true,
			hidden = function()
		...
		AddOption("goalupdateflash", {
			...
			set = function(_, v)
				Setter_Simple(_, v)
				if v then
					self.db.profile.goalcompletionflash = true
				end
		...
		AddOption("goalcompletionflash", {
			...
			get = function()
				return self.db.profile.goalcompletionflash --[[or self.db.profile.goalupdateflash--]]
			end,
			set = function(_, v)
				Setter_Simple(_, v)
				if not v then
					self.db.profile.goalupdateflash = false
				end
				ZGV:TryToCompleteStep()
			end,
			width = "single",
		...
		AddSubgroup("ants", {
			width = "single",
			hidden = function()
				return not self.db.profile.travel_system_advanced or not self.db.profile.pathfinding
			end,
		})
		...
		local function setrgb(colortable, r, g, b, a)
			...
			if type(r) == "table" then
				local rgbalpha = r
				colortable.r, colortable.g, colortable.b, colortable.a, colortable.alpha =
					rgbalpha.r, rgbalpha.g, rgbalpha.b, rgbalpha.a, rgbalpha.alpha
			else
		...
		AddOption("antspeed", {
			type = "select",
			disabled = function()
				return self.db.profile.waypointaddon ~= "internal" and self.db.profile.waypointaddon ~= "tomtom"
			end,
			values = {
				[1] = L["opt_antspeed_vslow"],
				[5] = L["opt_antspeed_slow"]:format(5),
				[10] = L["opt_antspeed_normal"]:format(10),
				[30] = L["opt_antspeed_fast"]:format(30),
				[999] = L["opt_antspeed_full"],
			},
			_default = 30,
		})
		...
		local antcolor_disabled = function()
			return false
		end
		...
		local antcolor_hidden = function()
			return not self.db.profile.multicolorants
		end
		...
		AddOption("colorantssingle", {
			...
			disabled = function()
				return not antcolor_hidden()
			end,
			get = function()
				return rgb2list(self.db.profile.colorantssingle)
			end,
			set = function(_, r, g, b, a)
		...
		AddOption("colorantsother", { --Add color to this table
			...
			get = function()
				return rgb2list(
					antcolor_disabled() and rgbalpha2rgba(ZGV.Pointer.Icons.ant_walk_default)
						or self.db.profile.colorantsother
				)
			end,
			set = function(_, r, g, b, a)
		...
		AddOption("colorantsfly", {
			...
			get = function()
				return rgb2list(
					antcolor_disabled() and rgbalpha2rgba(ZGV.Pointer.Icons.ant_flying_default)
						or self.db.profile.colorantsfly
				)
			end,
			set = function(_, r, g, b, a)
		...
		AddOption("colorantstaxi", {
			...
			get = function()
				return rgb2list(
					antcolor_disabled() and rgbalpha2rgba(ZGV.Pointer.Icons.ant_taxi_default)
						or self.db.profile.colorantstaxi
				)
			end,
			set = function(_, r, g, b, a)
		...
		AddOption("colorantsship", {
			...
			get = function()
				return rgb2list(
					antcolor_disabled() and rgbalpha2rgba(ZGV.Pointer.Icons.ant_ship_default)
						or self.db.profile.colorantsship
				)
			end,
			set = function(_, r, g, b, a)
		...
		AddOption("colorantsportal", {
			...
			get = function()
				return rgb2list(
					antcolor_disabled() and rgbalpha2rgba(ZGV.Pointer.Icons.ant_portal_default)
						or self.db.profile.colorantsportal
				)
			end,
			set = function(_, r, g, b, a)
		...
		AddOption("opacitytoggle", {
			type = "toggle",
			set = function(i, v)
				Setter_Simple(i, v) --[[self.db.profile.opacitymain = (v and 0.6 or 1.0)--]]
				self:ApplySkin()
		...
		AddOptionSpace()
		AddOption("gmfirstpage", {
			type = "select",
			values = {
				["1_home"] = L["opt_gmfirstpage_home"],
				["2_current"] = L["opt_gmfirstpage_current"],
				["3_recent"] = L["opt_gmfirstpage_recent"],
				["4_suggested"] = L["opt_gmfirstpage_suggested"],
				["5_last"] = L["opt_gmfirstpage_last"],
			},
			width = "full",
			pulloutWidth = "single",
			_default = "1_home",
		...
		do
			...
			AddOption("corpsearrow", {
				type = "toggle",
				disabled = function()
					return self.db.profile.waypointaddon == "none"
				end,
				_default = true,
				hidden = true,
				width = "double",
	...
	AddOptionGroup(
		...
		"DebugShare",
		"zgdebugshare",
		{ name = "Debug: share", guiHidden = not self.DEV or self.db.profile.hide_dev_once }
	)
	do
		AddOption("share_toggle", {
			name = function()
				return ZGV.db.profile.share_enabled and "Stop sharing" or "Start sharing"
			end,
			type = "execute",
			width = "full",
			disabled = function()
				return not ZGV.Sync:IsInGroup()
			end,
			func = function()
	...
	AddOptionGroup(
		...
		"DebugVariants",
		"zgdebugvaiants",
		{ name = "Debug: variants", guiHidden = not self.DEV or self.db.profile.hide_dev_once }
	)
	do
		AddOption("hidetracker", {
			type = "toggle",
			set = function(i, v)
				Setter_Simple(i, v)
				ZGV.Replacements:UpdateVisibility("force")
		...
		AddOption("gmshowoptions", {
			type = "toggle",
			name = "Guide Menu variant: show Options in top bar",
			set = function(i, v)
				Setter_Simple(i, v)
				ZGV.F.SetVisible(ZGV.GuideMenu.MainFrame.Header.Tabs.Options, v)
			end,
			width = "full",
			_default = false,
		})
		AddOption("gmshowoptionsleft", {
			type = "toggle",
			name = "Guide Menu variant: show Options in left panel",
			set = function(i, v)
				Setter_Simple(i, v)
				ZGV.F.SetVisible(ZGV.GuideMenu.MainFrame.MenuGuides.Options, v)
				ZGV.F.SetVisible(ZGV.GuideMenu.MainFrame.MenuGuides.OptionsDecor, v)
			end,
			width = "full",
			_default = false,
		})
		AddOption("debug_newicons", { type = "toggle", name = "Use new test icons", width = "full", _default = false })
		AddOption("start_on_closest_waypoint", {
			type = "toggle",
			width = "full",
			set = function(i, v)
				Setter_Simple(i, v)
				ZGV.Pointer:ResetCurrentWaypoint()
			end,
			_default = false,
		})
		AddOption("hijack_builtin_waypoint", {
			type = "toggle",
			width = "full",
			set = function(i, v)
				Setter_Simple(i, v)
		...
		AddOption("ga_removeilvl", {
			name = "Allow gear suggestions outside of the current item level",
			type = "toggle",
			width = "full",
			_default = false,
		})
		...
		AddOption("nc_toast_textsize", {
			...
			get = function(i, v)
				for k, v in ipairs(fontsizes) do
					if self.db.profile.nc_toast_fontsize == v then
						return k
					end
				end
				return 4
	...
	AddOption("load_all", {
		...
		set = function(info, val)
			Setter_Loud(info, val)
			self.db.profile.load_mail = val
			self.db.profile.load_im = val
			self.db.profile.load_betaguides = val
			self.db.profile.load_gold = val
			self.db.profile.show_ui = val
		end,
	})
	AddOption("show_gold", {
		type = "execute",
		desc = "Show Gold Guide",
		_default = false,
		func = function(info, val)
			ZGV.Gold:Show()
		end,
	})
	AddOption("show_appraiser", {
		type = "execute",
		desc = "Show Appraiser",
		_default = false,
		func = function(info, val)
			ZGV.Gold.Appraiser:Show()
		end,
	})
...
function ZGV:Options_RegisterDefaults()
	...
	end -- can work only once, as it CLEARS the _default fields!! (and it has to, as they're non-standard.)
	...
	if ZGV.ZTA and ZGV.ZTA.PrepareConfigDefaults then
		for g, o in pairs(ZGV.ZTA:PrepareConfigDefaults()) do
			for k, v in pairs(o) do
				defaults[g] = defaults[g] or {}
				defaults[g][k] = v
			end
		end
	end
	...
	if self.db.profile.arrowskin == "sheen" then
		self.db.profile.arrowskin = "fancy"
	end
	...
	if self.db.profile.fake_covenant == -1 then
		self.db.profile.fake_covenant = false
	end
	if self.db.profile.fake_covenant_feature_transport == -1 then
		self.db.profile.fake_covenant_feature_transport = false
	end
	...
	if self.db.char.BakedCache then
		if ZGV.db.profile.dumpmapdontcleansv then
			...
			StaticPopup_Show(
				"ZYGORGUIDESVIEWER_DEFAULT",
				"Baked cache data found.\nYou should run Baker now,\nif you haven't already."
			)
	...
	for flag, flagdata in pairs(df) do
		if type(flagdata) == "boolean" then
			df[flag] = { enabled = flagdata }
		end
	end -- convert
	for dfn, dfd in pairs(df) do
		dfd.color = dfd.color or defaults.profile.debug_flags[dfn] and defaults.profile.debug_flags[dfn].color
	end -- assign colors, if there weren't any
	...
	if not type(self.db.profile.ta_toggle_icons) == "boolean" then
		self.db.profile.ta_toggle_icons = true
	end
	...
	--]]
	if not _G[self.db.profile.debug_frame] then
		self.db.profile.debug_frame = nil
	end
			...
			for index = 1, #ZGV.ItemScore.Keywords do
				...
				if not stat.multi then
					if self.db.profile[prefix .. stat.blizz] then
						altDB[playername].statweights[prefixnew .. stat.blizz] =
							tonumber(self.db.profile[prefix .. stat.blizz])
					end
				end
...
function ZGV:Options_SetupPanels() -- Unused!
	...
	if self.db.profile.autogearframe ~= nil then
		self.db.profile.autogearauto = not self.db.profile.autogearframe
		self.db.profile.autogearframe = nil
	end
end
...
function ZGV:Options_SetupBlizConfig()
	...
	-- 			v.name ~= "cover" and self.optiontables.cover.name
	-- 		)
	-- 		panel.optiontable = self.optiontables[v.name]
	-- 		self.optionpanels[v.name == "main" and "" or v.name] = panel
	-- 		AceConfigDialog.BlizOptions[v.blizname][v.blizname]:SetCallback("default", function(group)
	-- 			ZGV:Options_ResetToDefaults(group)
	-- 		end)
	-- 	end
	-- end
...
function ZGV:GetCurrentGuideNum()
	...
	for i, data in pairs(ZygorGuidesViewer.registeredguides) do
		if data.title == self.CurrentGuide.title then
			return i
		end
	end
end
...
function ZGV:RefreshOptions()
	if ZGV.GuideMenu.MainFrame and ZGV.GuideMenu.MainFrame.WideColumnOptions.AceContainer:IsVisible() then
		LibStub("AceConfigDialog-3.0-Z"):Open(
			ZGV.GuideMenu.current_option,
			ZGV.GuideMenu.MainFrame.WideColumnOptions.AceContainer
		)
	end
end

function ZGV:SetOption(cat, cmd)
	LibStub("AceConfigCmd-3.0-Z").HandleCommand(
		self,
		"zygor",
		"ZygorGuidesViewer" .. (cat ~= "" and "-" .. cat or ""),
		cmd
	)
end
...
function ZGV:OPTTEST()
	ZGV.OPTTEST_F =
		ZGV.ChainCall(CreateFrame("FRAME", "ZGVOPTTEST_F")):SetSize(700, 500):SetPoint("CENTER", UIParent):Show().__END
	ZGV.OPTTEST_F.b = ZGV.ChainCall(ZGV.OPTTEST_F:CreateTexture()):SetAllPoints():SetColorTexture(0, 0, 0, 0.5).__END

	ZGV.OPTTEST_F.Content = ZGV.ChainCall(CreateFrame("FRAME", "ZGVOPTTEST_F2", ZGV.OPTTEST_F))
		:SetPoint("BOTTOMRIGHT")
		:SetPoint("TOPLEFT", ZGV.OPTTEST_F, "TOPLEFT", 200, 0).__END

	local F = ZGV.OPTTEST_F
	local C = F.Content
	local SCRF = LibStub("AceGUI-3.0-Z"):Create("ScrollFrame-Z")
	SCRF.frame:SetParent(C)
	SCRF.frame:SetAllPoints()
	...
	for i, v in ipairs(self.optiontables_ordered) do
		if v.name ~= "dev" then
			...
			if not lastB then
				...
				B:SetPoint("TOPLEFT", lastB, "BOTTOMLEFT")
			end
			lastB = B
...
SLASH_ZYGORWAY1 = "/zw"
SlashCmdList["ZYGORWAY"] = function(msg, editbox)
	ZGV:SetOption("Cover", "way " .. msg)
end
