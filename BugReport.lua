...
local table, string, tonumber, tostring, ipairs, pairs, setmetatable =
	table, string, tonumber, tostring, ipairs, pairs, setmetatable
...
local function CreateDumpFrameBasic()
	...
	local scroll = CHAIN(ui:Create("ScrollChild", frame, name .. "Scroll", "editbox"))
		...
		:HookScript("OnShow", function(me)
			me.child:SetFocus(true)
		end).__END
	frame.scroll = scroll

	scroll.child:SetScript("OnEscapePressed", function()
		frame.save = nil
		frame:Hide()
	end)
...
local function CreateDumpFrameReport()
	...
	local selector = CHAIN(ui:Create("DropDown", frame))
		...
		:SetWidth(200)
		:SetText(L["bugreport_bugtype"]).__END
	selector.frame:Show()
	...
	frame.header1 = CHAIN(frame:CreateFontString(nil, "OVERLAY"))
		...
		:SetJustifyV("TOP")
		:SetWordWrap(true)
		:SetText(L["bugreport_description"]).__END
	...
	CHAIN(frame.scroll1.child)
		...
		:SetScript("OnTabPressed", function()
			frame.edit2Box:SetFocus(true)
		end)
		:SetFont(FONT, 12, "")
	...
	frame.header2 = CHAIN(frame:CreateFontString(nil, "OVERLAY"))
		...
		:SetWidth(460)
		:SetWordWrap(true)
		:SetText(L["bugreport_info"]).__END
	...
	if not ZGV.StandAlone then
		frame.OKButton = CHAIN(ui:Create("Button", frame))
			...
			:SetScript("OnClick", function(self)
				local function safexml(s)
					return s:gsub("<", "&lt;"):gsub(">", "&gt;")
				end
				if frame.save and frame.timestamp and frame.text then
					local text = '<report timestamp="'
						.. frame.timestamp
						.. '">'
						.. "<type>"
						.. safexml(frame.selector.text:GetText())
						.. "</type>"
						.. "<description>"
						.. safexml(frame.edit1Box:GetText())
						.. "</description>"
						.. "<info>"
						.. safexml(frame.edit2Box:GetText())
						.. "</info>"
						.. "<body>"
						.. safexml(frame.text)
						.. "</body>"
	...
	frame.viewbut = CHAIN(ui:Create("Button", frame))
		...
		:SetPushedBackdropColor(unpack(SkinData("Accent"))).__END
...
BugReport.Flags =
	{ guide = 1, step = 1, player = 1, questlog = 1, inventory = 1, parselog = 1, itemscore = 1, gearfinder = 1 }

function BugReport:GetReport_Flavor()
	return (ZGV.IsRetail and "RETAIL")
		or (ZGV.IsClassicAnnivHardcore and "CLASSIC-ANNIVHC")
		or (ZGV.IsClassicAnniv and "CLASSIC-ANNIV")
...
function BugReport:GetReport_Travel()
	...
	if ZGV.db.profile.pathfinding and LibRover.RESULTS then
		...
		s = s .. "Travel mode: " .. ZGV.db.global.travelmode .. "\n"
		s = s .. "Comfort mode: " .. ZGV.db.profile.pathfinding_comfort .. "\n"
		if ZGV.db.profile.travelavoidportals then
			s = s .. "Avoid portals: true\n"
		else
			s = s .. "Avoid portals: false\n"
		end
		if ZGV.db.profile.travelusehs then
			s = s .. "Use HS: true\n"
		else
			s = s .. "Use HS: false\n"
		end
		if ZGV.db.profile.traveluseitems then
			s = s .. "Use items: true\n"
		else
			s = s .. "Use items: false\n"
		end
		if ZGV.db.profile.travelusespells then
			s = s .. "Use spells: true\n"
		else
			s = s .. "Use spells: false\n"
		end
		if ZGV.db.profile.travelprefertaxi then
			s = s .. "Prefer taxi: true\n"
		else
			s = s .. "Prefer taxi: false\n"
		end
		return s
	else
...
function BugReport:GetReport(maint, flags)
	...
	if self.Flags.guide then
		...
		if ZGV.CurrentGuide and not ZGV.CurrentGuide.parse_failed then
			...
			s = s
				.. ("Type: %s  Parsed: %s  Fully: %s\n"):format(
					tostring(g.type),
					g.parsed and "yes" or "no",
					g.fully_parsed and "yes" or "no"
				)
	...
	if self.Flags.step then
		...
		if step then
			for k, v in pairs(step) do
				...
				if k == "map" then
					f = (ZGV.GetMapNameByID(step.map) or "")
						.. "#"
						.. tostring(step.map)
						.. " /"
						.. tostring(step.floor)
			...
			s = s
				.. ("  (%s, %s, %s)\n"):format(
					complete and "COMPLETE" or "incomplete",
					possible and "POSSIBLE" or "impossible",
					step:IsAuxiliary() and "AUX" or "not aux"
				)
			...
			for i, goal in ipairs(step.goals) do
				...
				for k, v in pairs(goal) do
					...
					if
						...
						and k ~= "lasttext"
						and k ~= "lastbrief"
						and k ~= "lastshowcompleteness"
						and k ~= "mobs"
						and k ~= "target"
						and k ~= "targetid"
						and k ~= "objnum"
						and k ~= "condition_visible_raw"
						and k ~= "condition_complete_raw"
						and type(v) ~= "function"
					then
						f = anytostring(v)
					end
				...
				if goal.x or goal.y or goal.action == "goto" then
					local map
					if goal.map then
						map = (ZGV.GetMapNameByID(goal.map) or "")
							.. " #|cff888888"
							.. tostring(goal.map)
							.. "|r /|cffffaa00"
							.. tostring(goal.floor)
					...
					if goal.dist then
						s = s .. ("|cffff8800 %s%s|r"):format(goal.dist > 0 and ">" or "<", math.abs(goal.dist))
					else
						s = s .. "|cffff8800 (no dist)|r"
					end
					s = s .. "|r\n"
				end
				...
				if goal.questid and goal.quest then
					...
					s = s
						.. ('    quest: "%s" ##%s'):format(questdata and questdata.title or "?", tostring(goal.questid))
				...
				if goal:IsCompleteable() then
					...
					s = s
						.. (" (%s, %s, %s progress, %s, '%s'%s)\n"):format(
							comp and "|cff00ff00COMPLETE|r" or "|cffffaa00incomplete|r",
							poss and "|cffbbff00POSSIBLE|r" or "|cff880000impossible|r",
							tostring(prog),
							step:IsAuxiliary() and "|cff8888ffAUX|r" or "|cff8888aanot aux|r",
							tostring(explanation) or "-",
							isbad and " WARNING" or ""
						)
				else
	...
	if self.Flags.gearfinder then
		local s = ""
		s = s .. "\nGearfinder Dungeons\n"
		ZGV.ItemScore.GearFinder.CurrentExpansion = (GetClassicExpansionLevel and GetClassicExpansionLevel())
			or (GetServerExpansionLevel and GetServerExpansionLevel())
			or 1

		for dungeon, dungeondata in pairs(ZGV.ItemScore.Items) do
			local valid, future, ident, maxscale, mythic, comment = ZGV.ItemScore.GearFinder:IsValidDungeon(
				dungeondata.dungeonmap or dungeondata.dungeon,
				dungeondata.instanceId
			)
			s = s
				.. ("%s%s %s, "):format(
					dungeondata.dungeonmap or dungeondata.dungeon,
	...
	do
		...
		for i = 1, 30 do
			...
			if debuff then
				s = s .. ("%s (%d)\n"):format(debuff.name, debuff.spellId)
			end
		end
	...
	do
		...
		s = s .. "Profile:\n"
		local profile_s = ""
		for k, v in pairs(ZGV.db.profile) do
			profile_s = profile_s .. "  " .. k .. " = " .. anytostring(v) .. ",  "
		end
	...
	if ZGV.ErrorLogger_GetErrors then
		...
		if #errors > 0 then
			for ei, err in ipairs(errors) do
				s = s
					.. "----------------\nTime: "
					.. (err.time or "?")
					.. "\nCount: "
	...
	do
		local s = ""
		if ZGV.db.profile.sync_enabled then
			s = s .. "Sync enabled: true\n"
		else
			s = s .. "Sync enabled: false\n"
		end
		if ZGV.db.profile.share_masterslave == 0 then
			s = s .. "Sync mode: disabled\n"
		elseif ZGV.db.profile.share_masterslave == 1 then
			s = s .. "Sync mode: master\n"
		else
			s = s .. "Sync mode: slave\n"
		end
		report.sync = report.sync .. s
	end
	...
	for _, k in ipairs(sections) do
		local ss = report[k]
		if #ss > (limit_bypass[k] or limit) then
			ss = ss:sub(1, (limit_bypass[k] or limit))
				.. "\n ["
				.. #ss
				.. " chars, cut at "
				.. (limit_bypass[k] or limit)
				.. "]\n"
		end
		s = s .. ss
	end
...
function BugReport:GenerateAndShow(maint)
	...
	local author = maint and "zygor@zygorguides.com"
		or (ZGV.CurrentGuide and ZGV.CurrentGuide.author or L["report_noauthor"])
...
function BugReport:SaveDump(text, timestamp, header, addonmodule, severity, geardump)
	...
	if ZGV.DEV then
		if GuideRating.GuideRatingViewer then
			ZGV:Debug("Guide rating saved for upload.")
		else
			ZGV:Debug("Bug report saved for upload.")
			ZGV:ShowDump(ZGV.db.global.bugreports[timestamp])
		end
	end
end

function BugReport:PruneDumps()
	if not ZGV.db.global.bugreports then
		return
	end
	for timestamp, report in pairs(ZGV.db.global.bugreports) do
		if
			(time() - timestamp > 86400 * 7) -- remove week old
			or (#report == 0) -- remove empty?
			or (report:find("bug_report_sent=1")) -- remove sent
		then
			ZGV.db.global.bugreports[timestamp] = nil
		end
	end
end
...
function BugReport:SimpleDump(tab)
	...
	for k, v in pairs(tab) do
		tinsert(t, ("%s=%s"):format(k, (tonumber(v) and v or '"' .. tostring(v) .. '"')))
	end
...
function ZGV:ShowDump(text, title, options)
	...
	if type(text) == "table" then
		text = ZGV:Serialize(text)
	end
	text = tostring(text)

	if not self.BugReport.dumpFrameBasic then
	...
	f.editBox:SetText(text:gsub("[^%w%p%s]", function(c)
		return ("\\%d"):format(strbyte(c))
	end)) -- fix for out-of-ascii chars
...
function BugReport:DelayedShowReportDialog(report)
	...
	ZGV:ScheduleTimer(function()
		ZGV.BugReport:ShowReportDialog(report)
	end, 0.5)
end

function BugReport:ShowReportDialog(report)
	if not self.dumpFrameReport then
...
function StepFeedback:CreateFrame(itemlink)
	...
	frame.title = CHAIN(frame:CreateFontString())
		...
		:SetText(L["bugreport_step_title"]).__END
	...
	frame.severity =
		CHAIN(ui:Create("DropDown", frame.addoncontainer, 2, frame:GetFrameLevel() + 2)) --"Issue is minor and.."
	...
	frame.scroll.child:SetScript("OnEscapePressed", function()
		frame.save = nil
		frame:Hide()
	end)
	...
	frame.buttonsubmit = CHAIN(ui:Create("Button", frame))
		...
		:SetScript("OnClick", function(self)
			...
			if StepFeedback.ReportMode == "guide" then
				...
				if
					self:GetParent().editBox:GetText() ~= L["bugreport_step_message_addon"]
						and frame.addonmodule:GetCurrentSelectedItemValue() ~= "default"
						and frame.severity:GetCurrentSelectedItemValue() ~= "default"
					or ZGV.DEV
				then
					local geardump = ""
					if StepFeedback.itemlink then
						geardump = ZGV.ItemScore.Upgrades:GetGearReport(StepFeedback.itemlink)
						...
						ZGV:Print(StepFeedback.itemlink .. " will no longer be suggested as an upgrade.")
					...
					StepFeedback:Save(
						self:GetParent().editBox:GetText(),
						frame.addonmodule:GetCurrentSelectedItemValue(),
						frame.severity:GetCurrentSelectedItemValue(),
						geardump
					)
					frame.errEditbox:Hide()
					frame.errMessage:Hide()
					...
					if self:GetParent().editBox:GetText() == L["bugreport_step_message_addon"] then
						frame.errEditbox:Show()
					end
					if frame.addonmodule:GetCurrentSelectedItemValue() == "default" then
						frame.errComponent:Show()
					end
					if frame.severity:GetCurrentSelectedItemValue() == "default" then
						frame.errSeverity:Show()
					end
	...
	function frame:UpdateLayout(ftype, component, itemlink)
		...
		if StepFeedback.ReportMode == "guide" then
			...
			if
				frame.editBox:GetText() == L["bugreport_step_message_addon"]
				or string.match(frame.editBox:GetText(), "Please explain why you feel")
			then
				frame.editBox:SetText(L["bugreport_step_message"])
			end
			frame.editBox.placeholder = L["bugreport_step_message"]
			frame.errEditbox:Hide()
			frame.errMessage:Hide()
			...
			if itemlink then
				...
				if
					frame.editBox:GetText() == L["bugreport_step_message"]
					or string.match(frame.editBox:GetText(), "Please explain why you feel")
				then
					frame.editBox.placeholder = L["bugreport_step_message_addon"]
					frame.editBox:SetText(L["bugreport_step_message_addon"])
				end
			end
			frame.errEditbox:Hide()
			frame.errMessage:Hide()
...
function StepFeedback:Clear(ftype, component, itemlink)
	...
	if not ftype and not component then
		...
		if ftype then
			self.ReportMode = "addon"
		end
		if component then
			self.Frame.addonmodule:SetCurrentSelectedByValue(component)
		end
	end
...
function StepFeedback:GetGuideStepSignature()
	return (ZGV.CurrentGuide and ZGV.CurrentGuide.title or "??")
		.. "::"
		.. (ZGV.CurrentStep and ZGV.CurrentStep.num or "??")
end

function StepFeedback:GetStepReportHeader()
	return "bug_report_guide="
		.. self:GetGuideStepSignature()
		.. "\n"
		.. "bug_report_steplabel="
		.. (ZGV.CurrentStep and ZGV.CurrentStep.reportlabel)
		.. "\n"
		.. "bug_report_steptxt="
		.. (ZGV.CurrentStep and ZGV.CurrentStep:GetDebugDump():gsub("\n", ";;") or "")
		.. "\n"
		.. "bug_report_chain="
		.. ZGV.QuestDB:ExplainStep()
		.. "\n"
end
...
function StepFeedback:FindStepReportForCurrentStep(itemlink)
	...
	if string.match(self.Frame.editBox:GetText(), "Please explain why you feel") then
		return
	end
	local signature = self:GetGuideStepSignature() .. "\n" -- look for Name\\Of\\Guide::123\n
	for timestamp, report in pairs(ZGV.db.global.bugreports) do
		if report:find(signature, 1, true) then
			return timestamp, BugReport:GetDumpBody(report)
		end
	end
end
...
local function getNextGuideTitleSafely(where)
	...
	if not nextguide then
		return ZGV:Error("Cannot find next guide (" .. where .. "): " .. nextg)
	end
	return nextguide.title_short
end

function GuideRating:NextRatingGuide(source)
	local nextGuide = ZGV.CurrentGuide.next
	local scored = ZGV.db.char.scoredguides[nextGuide]

	if scored == "declined" or not nextGuide then ---Only if the guide popped up as a next invalid guide and was declined by the user
	...
	ZGV:SetGuide(nextGuide, nil, nil, nil, ZGV.CurrentGuide)
end
...
function GuideRating:CreateFrame()
	...
	frame = CHAIN(ZGV.UI:Create("Frame", nil)):SetHeight(220):SetBackdropColor(0, 0, 0, 0):CanDrag(false).__END
	...
	frame.scroll = CHAIN(ZGV.UI:Create("ScrollChild", frame, nil, "editbox"))
		...
		:HookScript("OnLoad", function(me)
			me.child:SetFocus(false)
		end).__END
	...
	frame.scroll.child:SetScript("OnLeave", function(self)
		GameTooltip:Hide()
	end)
	...
	frame.buttonsubmit = CHAIN(ZGV.UI:Create("Button", frame))
		...
		:SetScript("OnClick", function(self)
			if (GuideRating.score ~= nil) and (GuideRating.score > 0) then
				if
					(GuideRating.score == 1)
					and (frame.scroll.child:GetText() == "|cff888888Please tell us your thoughts on this guide.")
				then
		...
		:SetScript("OnClick", function(self)
			...
			if not GuideRating.CancelledRatingFrame then
				...
				GuideRating.CancelledRatingFrame.nextFS:SetText(
					nexttitle and L["viewer_special_cancelled_next"]:format("cfffe610", nexttitle)
	...
	frame.scroll.child:SetScript("OnEditFocusLost", function()
		if (GuideRating.score ~= nil) and (GuideRating.score > 0) then
			...
			elseif
				frame.scroll.child:GetText()
				~= (("|cff888888" .. (L["viewer_special_rateexp"] or "") or nil) and (GuideRating.score > 0))
			then
...
function GuideRating:CreateAltFrame()
	local frame = CHAIN(ZGV.UI:Create("Frame", nil))
		...
		:SetPoint("BOTTOMRIGHT", ZGV.Frame.Border).__END
	...
	frame.frameFS = CHAIN(frame:CreateFontString())
		...
		:SetJustifyH("CENTER")
		:SetText(L["viewer_special_rate_menu"]).__END
...
function GuideRating:CreateCancelledFrame()
	...
	frame.rateFS = CHAIN(frame.ratebutton:CreateFontString())
		...
		:SetPoint("BOTTOMRIGHT", frame.ratebutton, "BOTTOMRIGHT").__END
	...
	frame.nextFS = CHAIN(frame.nextstepbutton:CreateFontString())
		...
		:SetPoint("BOTTOMRIGHT", frame.nextstepbutton, "BOTTOMRIGHT").__END

	ZGV:ScheduleTimer(function()
		local nexttitle = getNextGuideTitleSafely("#003")
		frame.nextFS:SetText(
			nexttitle and L["viewer_special_cancelled_next"]:format("cfffe610", nexttitle)
				or L["viewer_special_cancelled_menu"]
		)
	end, 0.2)
...
function GuideRating:GetStepReportHeader()
	return "bug_report_guide="
		.. StepFeedback:GetGuideStepSignature()
		.. "\n"
		.. "bug_report_score="
		.. ZGV.BugReport.GuideRating.score
		.. "\n"
end
...
function GuideRating:HideRatingWidgets()
	...
	if GuideRating.NoRatingFrame then
		GuideRating.NoRatingFrame:Hide()
	end
	if GuideRating.CancelledRatingFrame then
		GuideRating.CancelledRatingFrame:Hide()
	end
	if GuideRating.ZygorPopupOn then
		GuideRating.ZygorPopupOn:Hide()
	end
	if GuideRating.ZygorPopup then
		GuideRating.ZygorPopup:Hide()
...
function GuideRating:ShowGuideRating()
	if ZGV.CurrentGuide and not ZGV.CurrentStep.score then
		return
	end
	if ZGV.BugReport.GuideRating.NoRatingFrame then
		ZGV.BugReport.GuideRating.NoRatingFrame:Hide()
	end
	if ZGV.BugReport.GuideRating.CancelledRatingFrame then
		GuideRating.CancelledRatingFrame:Hide()
	end

	if
		ZGV.db.char.scoredguides[ZGV.CurrentGuide.title] ~= nil
			and (ZGV.db.char.scoredguides[ZGV.CurrentGuide.title] > -1)
		or not ZGV.db.profile.ratings
		or ZGV.CurrentGuide.headerdata.noscoring
	then --if the guide is already rated (1-3), rating was refused (0) or ratings disabled in options or guide header
...
function GuideRating:ClearRateState()
	if GuideRating.GuideRatingViewer then
		...
		if
			ZGV.db.char.ratingfeedbackcache[ZGV.CurrentGuide.title] == nil
			and ZGV.db.char.ratingscorecache[ZGV.CurrentGuide.title] == nil
		then
...
function GuideRating:Popup()
	...
	if not GuideRating.ZygorPopupOn then
		...
		frame.frameFS = CHAIN(frame:CreateFontString())
			...
			:SetPoint("BOTTOMRIGHT", ZGV.Frame.Border)
			:SetJustifyH("CENTER")
			:SetText(L["viewer_special_senttopopup"]).__END
	end
...
function GuideRating:UpdateText()
	if ZGV.Frame.Border:GetHeight() >= 274 or not ZGV.db.profile.fixedheight then
		GuideRating.GuideRatingViewer.frameFS:SetText(
			L["viewer_special_rate"]:format("cfffe610", ZGV.CurrentGuide.title_short)
		)
		GuideRating.GuideRatingViewer:Show()
	else
		GuideRating.GuideRatingViewer.frameFS:SetText(
			L["viewer_special_popup"]:format("cfffe610", ZGV.CurrentGuide.title_short)
		)
		GuideRating.GuideRatingViewer:Show()
	end
end
