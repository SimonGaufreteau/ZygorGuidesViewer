...
GOALTYPES['_item'] = {
	parse = function(self,params)		-- serves as generic parser for: [num] [name]##id["+"]
		...
		if self.targets and (self.targets[1] and self.targets[1][1]) then
			-- find common target name, if possible

			-- Build all possible word sequences from first string (n words, n-1 words, n-2 words...)
			local test_words = {strsplit(" ",self.targets[1][1])}
			local result = ""
			for length = #test_words, 1, -1 do
				for start = 1, #test_words - length + 1 do
					local test_sequence = ""
					for i = start, start + length - 1 do
						test_sequence = test_sequence .. test_words[i] .. " "
					end
					...
					-- If this sequence is common and longer than the current longest, update it
					if is_common and #test_sequence>#result then
			...
			if result ~= "" then
				self.targetcommon = result:sub(0,-2) -- strip trailing space
			end
		end
...
GOALTYPES['kill'] = {
	parse = function(self,params)
		...
		if self.targets and self.targetcommon then
			self.targetcommon = self.targetcommon.." enemies"
		end
	end,
...
GOALTYPES['learnpet'] = {
	...
	gettext = function(self) return L["stepgoal_learnpet"]:format(COLOR_ITEM(ZGV.Localizers:GetTranslatedNPC(tonumber(self.useid or self.npcId),self.usename or self.pet))) end,
}
...
GOALTYPES['petlevel'] = {
	...
	gettext = function(self) return L["stepgoal_petlevel"]:format(COLOR_ITEM(ZGV.Localizers:GetTranslatedNPC(tonumber(self.useid or self.npcId),self.pet)),self.usename or self.level) end,
}
...
GOALTYPES['noquest'] = {
	...
	gettext = function(self)
		return (L["stepgoal_noquest"]):format(COLOR_NPC(ZGV.Localizers:GetTranslatedNPC(tonumber(self.useid or self.npcid),self.usename or self.npc)))
	end,
	click_to_complete = true,
}
	...
	onevent = function(self, _, event, ...)
		ZGV:ScheduleTimer(function()
			if MerchantFrame and MerchantFrame:IsVisible() then -- from MERCHANT_SHOW and INIT
				self.propernpc = (ZGV.GetUnitId("NPC")==self.npcid)
			end
		end,0)

		if event=="MERCHANT_CLOSED" then 
			if self.propernpc then
	...
	parse = function(self,params)
		...
		if spell then
			...
			if level then
				...
				if total then
					if ZGV.Professions.SpellToItem then 
						self.itemid = ZGV.Professions.SpellToItem[self.spellid]
						self.targetid = self.itemid
					end
...
GOALTYPES['repcollect'] = {
	parse = function(self,params)
		self.params=params
		self.item,self.turnincount,self.turningain,self.faction,self.rep = params:match("^(.*),(%d*),(%d*),(.*),(.*)$")
	...
	gettext = function(self,complete,complete_extra,goalcountnow,goalcountneeded,remaining,brief) 
		local itemname = self.targetcommon or ZGV:GetItemInfo(self.targetid)
 
		if complete then 
			return L["stepgoal_collectrep_done"]:format(COLOR_ITEM(itemname),self.repname,self.faction)
		else
			local goalcountnow = C_Item.GetItemCount(self.targetid or 0)
			if self.targets then
				goalcountnow = 0
				for _,subitem in ipairs(self.targets) do
					goalcountnow = goalcountnow + C_Item.GetItemCount(subitem[2])
				end
			end
			...
			return L["stepgoal_collectrep"]:format(COLOR_COUNT(remaining),COLOR_ITEM(itemname),self.repname,self.faction),nil,progtext
		end
	end,
...
GOALTYPES['click'] = {
	parse = function(self,params)
		GOALTYPES['_item'].parse(self,params)
		if self.targets and self.targetcommon then
			self.targetcommon = self.targetcommon.." objects"
		end
	end,
	gettext = function(self,complete,complete_extra,goalcountnow,goalcountneeded,remaining,brief) 
		if self.targets and #self.targets>1 then
...
local gossipicon = (ZGV.IsRetail and 136458) or 132052
GOALTYPES['home'] = {
	parse = function(self,params)
		self.param = params
		...
		if event=="GOSSIP_SHOW" then 
			for _,goal in ipairs(ZGV.CurrentStep.goals) do
				if goal.action=="talk" and goal.npcid==ZGV.GetUnitId("npc") then
					...
					for _,gossip in ipairs(gossips) do
						if gossip.icon==gossipicon then
							C_GossipInfo.SelectOption(gossip.gossipOptionID)
						end
					end
					return
...
GOALTYPES['goto'] = {
	parse = function(self,params,step,prevmap,prevfloor,indoors_flag)
		...
		if err and ZGV.DEV then return err end
...
GOALTYPES['onlyinsticky'] = {
	parse = function(self) self.onlyinsticky=true end,
}
...
GOALTYPES['count'] = {
	parse = function(self,params)
		local value,displaystyle = params:match("^([0-9]+) ([a-z]+)$") -- optional modifiers
...
GOALTYPES['discover'] = {
	parse = function(self,params)
		self.zone = params
		if self.zone:sub(1,4)=="the " then 
			if GetLocale()=="enUS" then self.the = "the " end
			self.zone = self.zone:sub(5,-1)
		end

		self.pattern = ERR_ZONE_EXPLORED:gsub("%%s","(.*)")
	end,
	events = {"UI_INFO_MESSAGE"},
	onevent = function(self, _, event, errorType, message)
		if errorType==396 then
			local zonename = message:match(self.pattern) 
			local engname = ZGV.BZR[zonename] or zonename or ""
			self.seenzone = engname
		end
	end,
	iscomplete = function(self)
		return self.seenzone == self.zone,true
	end,
...
function Goal:IsCompleteable()
	...
	if self.action=="goto" then
		...
		local lastgoto
		for i,goal in ipairs(self.parentStep.goals) do
			if goal.action=="goto" and not goal.force_complete and not goal.force_nocomplete then  -- a goto with no extra flavours, or something that is not a goal (tip, etc)
				lastgoto=goal
			elseif goal.action then
				all_gotos=false
				break
			end
		end
...
function Goal:IsComplete()
	...
	--if self.skipoptional and ZGV.db.profile.skipoptional then return false,false,0,1 end -- goal is optional, and option to skip is enabled
...
function Goal:UnregisterEvents()
	local GOAL = GOALTYPES[self.action]
	if self.eventfunc then
		for _,event in ipairs(GOAL.events) do
			ZGV:RemoveEventHandler(event,self.eventfunc)
...
function Goal:AutoTranslate()
	...
	if self.targetid then
		...
		if self.action=="kill" then
			if ZygorGuidesNPCs then
				local target,tooltip=ZGV.Localizers:GetTranslatedNPC(tonumber(self.useid or self.targetid),self.usename or self.target)
				if target then
	...
	if self.npcid then
		local npc,tooltip=ZGV.Localizers:GetTranslatedNPC(tonumber(self.useid or self.npcid),self.usename or self.npc)
		--if npc then
...
function Goal:GetText(showcompleteness,brief,showtotals,nocolor)
	if not self.prepared then self:Prepare() end
	if self.action=="mapmarker" then return "" end -- they are hidden, always
	...
	if showcompleteness then
		...
		elseif self.questid then
			goalcountnow,goalcountneeded,remaining = Goal.GetQuestGoalData(self.questid,self.objnum) -- self.count removed, we no longer want to override visible nums
		...
		--if complete and (goalcountnow or 0)<(goalcountneeded or self.count or 0) then goalcountnow=goalcountneeded or self.count end -- we can show 1/n as compelted now, don't override this
	end
	...
	elseif self.action=='from' or self.action=='avoid' then
		...
		for i,mob in ipairs(self.mobs) do
			local mobname
			if mob.id then mobname=ZGV.Localizers:GetTranslatedNPC(tonumber(self.useid or mob.id),self.usename or mob.name) end
			if mobname then
				mob.name = mobname
	...
	if showtotals and not (self.displaystyle=="hidden") then
		if not progtext then
			...
			elseif goalcountneeded and goalcountneeded==0 then 
				progtext=L["completion_count"]:format(goalcountnow)
			end
			...
			if self.displaystyle=="percent" and goalcountnow then
				progtext=L["completion_perc"]:format(goalcountnow)
			end
		end
