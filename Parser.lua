...
local ConditionEnv = {
	...
	_Setup = function(self)
		...
		for i=1,25 do registerLiteral("Renown"..i,i) end
	...
	counthaveq = function(...)
		...
		for i = 1, count do
			local id = select(i, ...)
			if Parser.ConditionEnv.haveq(id) then res = res + 1 end
		end
		return res
	end,
		...
		for i = 1, count do
			...
			elseif tonumber(id) then
				...
				if not (q and (q.complete and not q.inlog)) then -- some quests remain active/have time for up to 60 seconds after turning in. verify that we did not just turn that in. at the same time, C.QuestLog.IsComplete returns false as soon as we turn them in, so we can't use that
					if not (settings and settings.skipactive) and C_TaskQuest.IsActive(id) then return true end -- it is active, good
					if not (settings and settings.skiptime) and C_TaskQuest.GetQuestTimeLeftSeconds(id) then return true end -- fallback, does it have time remaining
				end
			else
				for _,qid in ipairs(ParseRanges(id)) do
					local q = ZGV.questsbyid[qid]
					if not (q and (q.complete and not q.inlog)) then -- some quests remain active/have time for up to 60 seconds after turning in. verify that we did not just turn that in. at the same time, C.QuestLog.IsComplete returns false as soon as we turn them in, so we can't use that
						if not (settings and settings.skipactive) and C_TaskQuest.IsActive(qid) then return true end -- it is active, good
						if not (settings and settings.skiptime) and C_TaskQuest.GetQuestTimeLeftSeconds(qid) then return true end -- fallback, does it have time remaining
					end
				end
			end
		end
	...
	questonmap = function(...)
		...
		if type(maps)=="number" then maps={maps} end
		...
		for qid,_ in pairs(quests) do
			local q=ZGV.questsbyid[qid]
			if (q and q.inlog) then return true,qid,"accepted" end
		end


		for _,mapID in ipairs(maps) do
			local taskquests = C_TaskQuest.GetQuestsOnMap(mapID)
			if not taskquests then return true end -- not loaded, wait for proper
...
function Parser:ParseHeader(guide)
	...
	if guide.headerdata.parsed then return end
