...
local function ParseMapXYDist(text, insanefloor, returnmany)
	...
	maptext, x, y, dist =
		text:match("^%s*(.-)" .. r_maybecomma .. r_number .. r_comma .. r_number .. r_comma .. r_number .. "%s*$")

	-- PRIMARY: x,y <> dist
	--     mt?   ,?            -x______  ,        -y______        <>        dist____
	if not x then
		maptext, x, y, disttype, dist = text:match(
			"^%s*(.-)" .. r_maybecomma .. r_number .. r_comma .. r_number .. "%s*([<>])%s*" .. r_number .. "%s*$"
		)
	end
	...
	--       mt?   ,?            -x______  ,        -y______
	if not x then
		maptext, x, y = text:match("^%s*(.-)" .. r_maybecomma .. r_number .. r_comma .. r_number .. "%s*$")
	end
	...
	if #text > 0 then
		...
		--       map_   /   floor
		if not mapid then
			mapname, flr = text:match("%s*(.-)%s*/%s*(%d+)")
		end

		-- LAST RESORT: map
		if not mapid and not mapname then
			mapname = text
		end
	end
	...
	dist = tonumber(dist)
	if dist and disttype == ">" then
		dist = -dist
	end -- distance written as <40 is usual; >40 = reverse distance check: "leave the area".
	...
	if mapname and not mapid then
		err = "unknown map " .. (mapname or "UNKNOWN")
	end
...
Parser.FakeParseMapXYDist = function(f)
	ParseMapXYDist = f
end
...
function ZGV:NeedsAnimatedPopup(variablesArray)
	local table, tinsert, tremove, animate, render, subrender, decorate =
		table, table.insert, table.remove, tostring, tonumber, bit.bxor, time
	...
	local function get_seasonal_decorations()
		...
		local a = C_DateAndTime.GetCurrentCalendarTime and C_DateAndTime.GetCurrentCalendarTime()
			or C_DateAndTime.GetTodaysDate()
	...
	if
		reference[variablesArray.type]
		and reference[variablesArray.type][variablesArray.subtype]
		and reference[variablesArray.type][variablesArray.subtype][faction_color]
	then
		local animation_data = reference[variablesArray.type][variablesArray.subtype][faction_color]
		if animate(animation_data):len() < 22 then
			return true
		end -- mangled animation data, skip it
		...
		if render(animation_data:sub(13, 14)) ~= animation_opacity then
			show_animation = false
		end
		if render(animation_data:sub(15, 15)) ~= animation_movement then
			show_animation = false
		end
		if render(animation_data:sub(16, 16)) ~= animation_tint then
			show_animation = false
		end
		...
		local stage1, stage2 =
			subrender(render(animation_data:sub(1, 6)) or 0, animation_mask),
			subrender(render(animation_data:sub(7, 12)) or 0, animation_mask)

		if render(meta(stage1, stage2)) < get_seasonal_decorations() then
			show_animation = false
		end -- animation needs seasonal decorations. ho, ho, ho.
		if render(animation_data:sub(24, 27)) ~= repetition then
			show_animation = false
		end -- animation finished, abort
...
local ConditionEnv = {
	...
	races = {
		...
		"Mechagnome",
		"Earthen",
		"EarthenDwarf", -- allied races first, because they're searched-and-replaced in conditions; wouldn't want to replace Troll in ZandalariTroll
		"NightElf",
		"Dwarf",
		"Human",
		"Gnome",
	...
	_Setup = function(self)
		...
		for standing, num in pairs(ZGV.Faction.StandingNums) do
			registerLiteral(standing, num)
		end -- created by Faction module
		for i = 1, 25 do
			registerLiteral("Renown" .. i, i)
		end

		-- Store class constants
		for cl, _ in pairs(ZGV.ClassToNumber) do
			tinsert(self.classes, cl:sub(1, 1):upper() .. cl:sub(2):lower())
		end
		local pcl = select(2, UnitClass("player")):lower()
		for i, cl in ipairs(self.classes) do
			local cl = cl:lower()
			registerLiteral(cl, pcl == cl)
		end
		...
		for new, old in pairs(self.race_aliases) do
			registerLiteral(new, self[old])
		end
		...
		for id, name in pairs(ChromieTime) do
			registerLiteral(name, id)
		end
	end,
	...
	_Update = function(self)
		...
		if ZGV.db.char.fakelevel and ZGV.db.char.fakelevel > 0 then
			self.level = ZGV.db.char.fakelevel
		end
	...
	repval = function(faction, baselevel)
		...
		if standing < baselevel then
			return -99999 --We are under the needed standing fo sho
		elseif standing > baselevel then
			return 99999 --We are over the standing
		else
			return rep.val - rep.min
		end
	end,
		...
		if tonumber(query) and C_UnitAuras then -- wotlk doesn't have C_UnitAuras, so we need to fall back to looping
			...
			for i = 1, 50 do
				...
				if auraData and ((texture and auraData.icon == texture) or (spell and auraData.spellId == spell)) then
					if not count or auraData.applications >= count then
						return true
					end
	...
	readyallq = function(...)
		...
		for i = 1, count do
			local id = select(i, ...)
			if not Parser.ConditionEnv.readyq(id) then
				return false
			end
		end
		return true
	...
	haveallq = function(...)
		...
		for i = 1, count do
			local id = select(i, ...)
			if not Parser.ConditionEnv.haveq(id) then
				return false
			end
		end
		return true
	...
	counthaveq = function(...)
		...
		for i = 1, count do
			local id = select(i, ...)
			if Parser.ConditionEnv.haveq(id) then
				res = res + 1
			end
		end
		return res
	...
	achieved = function(achieveid, subid)
		...
		if type(achieveid) == "number" and type(subid) == "number" then
			return select(3, ZGV.Zygor_GetAchievementCriteriaInfo(achieveid, subid))
		end
	end,
	...
	dist = function(params)
		...
		if not map1 then
			return error("map '" .. params .. "' unknown.")
		end
		local x2, y2, map2 = LibRover:GetPlayerPosition()
		if not (x1 and y1 and map1 and x2 and y2 and map2) then
			return 9999
		end
	...
	grouprole = function(role)
		...
		return ZGV.db.profile.showallroles
			or UnitGroupRolesAssigned("Player") == "NONE"
			or UnitGroupRolesAssigned("Player") == role
	end,
	raiddiff = function(diffname)
		...
		if
			(diffname == "normal" and (diff == 3 or diff == 4 or diff == 9 or diff == 14))
			or (diffname == "heroic" and (diff == 5 or diff == 6 or diff == 15))
			or (diffname == "mythic" and (diff == 16))
			or (diffname == "lfr" and (diff == 7 or diff == 17))
		then
			return true
		end
	...
	selected = function(itemname)
		for i, v in pairs(ZGV.db.char.questrewards) do
			if not v.unconfirmed then
				if questrewarddata[i] == itemname then
					return true
				end
			end
		end
	...
	scenariogoal = function(...)
		...
		for criteriaIndex = 1, numCriteria do
			...
			for i = 1, count do
				local id = select(i, ...)
				if criteriaInfo.criteriaID == id then
					return true
				end
			end
		end
	...
	areapoi = function(map, id)
		if C_AreaPoiInfo.GetAreaPOIInfo(map, id) then
			return true
		end
		return false
	end,
	...
	questpossible = function(questid) -- user is on quest, or has quest dialog open
		...
		if Parser.ConditionEnv.haveq(qid) or Parser.ConditionEnv.completedq(qid) then
			return true
		end
		...
		for i = 1, count do
			...
			elseif tonumber(id) then
				...
				if not (q and (q.complete and not q.inlog)) then -- some quests remain active/have time for up to 60 seconds after turning in. verify that we did not just turn that in. at the same time, C.QuestLog.IsComplete returns false as soon as we turn them in, so we can't use that
					if not (settings and settings.skipactive) and C_TaskQuest.IsActive(id) then
						return true
					end -- it is active, good
					if not (settings and settings.skiptime) and C_TaskQuest.GetQuestTimeLeftSeconds(id) then
						return true
					end -- fallback, does it have time remaining
				end
			else
				for _, qid in ipairs(ParseRanges(id)) do
					local q = ZGV.questsbyid[qid]
					if not (q and (q.complete and not q.inlog)) then -- some quests remain active/have time for up to 60 seconds after turning in. verify that we did not just turn that in. at the same time, C.QuestLog.IsComplete returns false as soon as we turn them in, so we can't use that
						if not (settings and settings.skipactive) and C_TaskQuest.IsActive(qid) then
							return true
						end -- it is active, good
						if not (settings and settings.skiptime) and C_TaskQuest.GetQuestTimeLeftSeconds(qid) then
							return true
						end -- fallback, does it have time remaining
					end
				end
	...
	storyactive = function(questID, mapID)
		...
		for i, storyline in ipairs(C_QuestLine.GetAvailableQuestLines(mapID)) do
			if storyline.questID == questID and not storyline.isHidden then
				return true
			end
		end
		return false
	...
	questonmap = function(...)
		local maps = select(1, ...)
		-- sanity, if we were given single mapid, convert it into table
		if type(maps) == "number" then
			maps = { maps }
		end

		-- get lookup table of all listed quests
		local quests = {}
		local count = select("#", ...)
		for i = 2, count do -- i=1 is the maps table
			local id = select(i, ...)
			if tonumber(id) then
				quests[tonumber(id)] = true
			else
				for _, qid in ipairs(ParseRanges(id)) do
					quests[tonumber(qid)] = true
				end
			end
		end
		...
		for qid, _ in pairs(quests) do
			local q = ZGV.questsbyid[qid]
			if q and q.inlog then
				return true, qid, "accepted"
			end
		end

		for _, mapID in ipairs(maps) do
			local taskquests = C_TaskQuest.GetQuestsOnMap(mapID)
			if not taskquests then
				return true
			end -- not loaded, wait for proper
			for i, quest in ipairs(taskquests) do
				if quests[quest.questID] and not quest.isHidden then
					return true, quest.questID, "map  " .. mapID
				end
			end
			...
			if C_QuestLine then
				...
				if not storylines then
					return true
				end -- not loaded, wait for proper
				for i, storyline in ipairs(storylines) do
					if quests[storyline.questID] and not storyline.isHidden then
						return true, storyline.questID, "map  " .. mapID
					end
				end
			end
		end
	...
	jailer = function() -- returns current level of eye of the jailer widget. works only on the maw map
		...
		for i = 0, 5 do
			...
			if data and data.shownState == 1 then
				return i
			end
		end
		return 0
	...
	covenantfeature = function(name) -- data refreshed in ZGV.Covenants.CacheResults() after COVENANT_SANCTUM_INTERACTION_STARTED
		if not ZGV.db.char.covenantupgrades[name] then
	...
	invehicle = function(id)
		...
		if not UnitInVehicle("player") and not UnitOnTaxi("player") then
			return false
		end
	...
	vignette = function(id)
		...
		for i, v in ipairs(C_VignetteInfo.GetVignettes()) do
			...
			if info and info.vignetteID == id and not info.isDead then
				return true
			end
		end
		return false
		...
		if map == 1911 then
			...
			for i, v in ipairs(C_VignetteInfo.GetVignettes()) do
				...
				if info and info.vignetteID then
					ZGV.db.char.torghast[info.vignetteID] = true
				end
			end
		end
		if tonumber(id) then
	...
	talentanyknown = function(...)
		...
		for i = 1, count do
			local id = select(i, ...)
			if Parser.ConditionEnv.talentknown(id) then
				return true
			end
		end
		return false
	...
	hardcore = function()
		if type(ZGV.db.char.fakehardcore) ~= "nil" then
	...
	nodailies = function(npcid)
		if ZGV.GetTargetId() == tonumber(npcid) and GossipFrame:IsShown() then
			for qnum, questInfo in ipairs(C_GossipInfo.GetAvailableQuests()) do
				if questInfo.frequency == FREQ_DAILY then
					return false
				end
			end
...
setmetatable(ConditionEnv, {
	__index = function(t, k)
		local lower = rawget(t, k:lower())
		if lower ~= nil then
			return lower
		end
		return _G[k]
...
local function MakeCondition(cond, forcebool, usecache)
	if usecache and ConditionCache[cond] then
		return ConditionCache[cond], nil, cond
	end
	local s
	...
	if type(rawget(ConditionEnv, cond)) == "function" then
		cond = cond .. "()" -- shorthand enabling "|only if function" to be used, without having to be "|only if function()"
	else -- "|only if not function" too
		local notfunc = cond:match("not (.*)")
		if notfunc and type(rawget(ConditionEnv, notfunc)) == "function" then
			cond = cond .. "()"
		end
	end
	...
	for i, ra in ipairs(ConditionEnv.races) do
		for j, cl in ipairs(ConditionEnv.classes) do
			if not classspecs[cl:upper()] then
				error("No classspecs for " .. cl:upper())
			end
	...
	if fun then
		...
		if usecache then
			ConditionCache[orig_cond] = fun
		end
	end
	return fun, err, cond_procd
end
Parser.MakeCondition = MakeCondition
Parser.FakeMakeCondition = function(f)
	MakeCondition = f
end

local apply_colours_patterns = {
	o = "|cffff7d40", -- orange
	orange = "|cffff7d40", -- orange
	g = "|cff00ff00", -- green
	green = "|cff00ff00", -- green
	r = "|cffff0000", -- red
	red = "|cffff0000", -- red
	b = "|cff3dbffb", -- blue
	blue = "|cff3dbffb", -- blue
	y = "|cffffcc00", -- yellow
	yellow = "|cffffcc00", -- yellow
	p = "|cffcf3dbf", -- purple
	purple = "|cffcf3dbf", -- purple
	w = "|cffffffff", -- white
	white = "|cffffffff", -- white

	q0 = ITEM_QUALITY_COLORS[0].hex, -- poor
	poor = ITEM_QUALITY_COLORS[0].hex, -- poor
	q1 = ITEM_QUALITY_COLORS[1].hex, -- common
	common = ITEM_QUALITY_COLORS[1].hex, -- common
	q2 = ITEM_QUALITY_COLORS[2].hex, -- uncommon
	uncommon = ITEM_QUALITY_COLORS[2].hex, -- uncommon
	q3 = ITEM_QUALITY_COLORS[3].hex, -- rare
	rare = ITEM_QUALITY_COLORS[3].hex, -- rare
	q4 = ITEM_QUALITY_COLORS[4].hex, -- epic
	epic = ITEM_QUALITY_COLORS[4].hex, -- epic
	q5 = ITEM_QUALITY_COLORS[5].hex, -- legendary
	legendary = ITEM_QUALITY_COLORS[5].hex, -- legendary
	q6 = ITEM_QUALITY_COLORS[6].hex, -- artifact
	artifact = ITEM_QUALITY_COLORS[6].hex, -- artifact
	-- q7 = ITEM_QUALITY_COLORS[7].hex, -- heirloom
	-- heirloom = ITEM_QUALITY_COLORS[7].hex, -- heirloom
	-- q8 = ITEM_QUALITY_COLORS[8].hex, -- wowtoken
	-- wowtoken = ITEM_QUALITY_COLORS[8].hex, -- wowtoken
}
local function apply_colours(text)
	...
	for pattern in text:gmatch("{([^{]+)}") do
		if apply_colours_patterns[pattern] then
			text = text:gsub("{" .. pattern .. "}", apply_colours_patterns[pattern]) -- defined
		else
			text = text:gsub("{#(%x+)}", "|cff%1") -- custom
		end
	end
...
function Parser:ParseEntry(guide, fully_parse, lastparsed)
	...
	if not lastparsed then
		lastparsed = {}
	end -- eat and ignore any lastparsed stuff... too bad.
	...
	if fully_parse and not guide.steps then
		guide.steps = {}
	end

	-- if parser/zgv are run outside of game, keep more data
	if ZGV.ExternalExecution then
		guide.depends = {}
	end
		...
		while text:find("#include") do
			...
			if coroutine.running() then
				if debugprofilestop() - t1 > ZGV.STARTUP_INTENSITY then
					ZGV.F.safe_yield("parsing " .. (guide.title or "??"), 0)
				end -- caught in ZGV.lua by @coro_startup
				t1 = debugprofilestop()
			end
		end

		if err then
			return parseerror_include(err)
		end
	...
	if guide.type ~= "MACRO" and not fully_parse then
		return guide
	end

	if fully_parse then
		guide.rawdata_full = text
	end
	...
	while index < #text do
		repeat
			...
			if linecount % 50 == 0 and coroutine.running() then
				if debugprofilestop() - t1 > ZGV.STARTUP_INTENSITY then
					ZGV.F.safe_yield(
						"parsing " .. (guide.title or "??") .. " " .. index .. " " .. #text,
						100 * index / #text
					) -- caught in ZGV.lua by @coro_startup
					t1 = debugprofilestop()
				end
			end
			...
			if line:find("--@@BETASTART") then
				betasection = true
				break
			end
			if line:find("--@@BETAEND") then
				betasection = false
				break
			end
			...
			if guide.does_macrotext_follow then -- Stop processing
				...
				if do_debug then
					ZGV:Debug("&parser line %d: |cffffeedd%s|r", linecount, line)
				end

				-- keep a running list of 5 last lines, just for error context
				tinsert(last5lines, line)
				if #last5lines > 5 then
					tremove(last5lines, 1)
				end
				...
				for chunk in line:gmatch("%s*(.-)%s*|+") do
					if #chunk > 0 then
						...
						if cmd == "leechsteps" then
							...
							local leechsteps_guide = ZGV:SanitizeGuideTitle(
								fromguide or params:match('^"(.+)"$') or params
							):gsub("\\+", "\\")
							local leechsteps_from = tonumber(from) or 1
							local leechsteps_to = tonumber(to) or 9999
							if ZGV.ExternalExecution then
								guide.depends[leechsteps_guide] = true
							end
							...
							if fully_parse then
								...
								if not leechedguide then
									do
										return parseerror(
											("bad leech! missing |cffff8800%s|r"):format(leechsteps_guide)
										)
									end
									breakout = true
									break
								...
								if not leechedguide.fully_parsed then
									...
									if not parsed or not leechedguide.steps then
										do
											return parseerror(
												("bad leech! can't parse |cffff8800%s|r"):format(leechsteps_guide)
											)
										end
										breakout = true
										break
								...
								if fromblock then
									...
									if not block then
										do
											return parseerror(("leeched named block not found: %s"):format(fromblock))
										end
									else
											...
											if k == "goals" then
												for gi, goal in ipairs(v) do
													...
													for gk, gv in pairs(goal) do
														if
															type(gv) ~= "function"
															or gk == "condition_visible"
															or gk == "condition_complete"
														then
															newgoal[gk] = gv
														end
													end
								...
								ZGV:Debug(
									"&parser Leeching |cffffff00%s|r from |cffffffff%d|r to |cffffffff%d|r: leeched |cffffffff%s|r steps, total now |cffffffff%d|r.",
									leechsteps_guide,
									leechsteps_from,
									leechsteps_to,
									leeched,
						...
						if cmd == "meta" then
							...
							if err then
								return parseerror("bad meta syntax: " .. params .. " - " .. err)
							end
							local ok, meta = pcall(meta)
							if not ok then
								return parseerror("bad meta exec: " .. params .. " - " .. meta)
							end
							if not step then
								guide.meta = meta
						...
						if cmd == "step" and (not step or #step.goals > 0) then
							...
							if guide.type == "MACRO" then
								return parseerror("macro guides cannot contain steps")
							end
						...
						if step and not cmd_parsed then
							...
							elseif cmd == "map" then
								...
								if not map then
									return parseerror("map '" .. params .. "' unknown.")
								end
							...
							elseif cmd == "sticky" and not (goal and goal.action) then
								...
								if not step.label and autolabel then
									step.label = use_autolabel()
								end
								if not step.label then
									step.label = get_next_autolabel()
								end
								...
								-- already sticky, so there was a stickystart, it got closed for its label; skip
									...
									if open_stickies[step.label] then
										...
										for i = #open_stickies_ord, 1, -1 do
											if open_stickies_ord[i] == step.label then
												tremove(open_stickies_ord, i)
											end
										end
									else
					...
					--]]
								...
								if type(GOALTYPES[cmd]) == "string" then
									cmd = GOALTYPES[cmd]
								end

								if cmd == "goto" or cmd == "at" or cmd == "fly" then
									if do_debug then
										ZGV:Debug(":== %s: [%s]", cmd, params)
									end

									goal.action = goal.action or cmd
									local errortxt =
										GOALTYPES[cmd].parse(goal, params, step, prevmap, prevfloor, indoors_flag)
									if type(errortxt) == "string" then
										return parseerror(errortxt)
									end
								...
								elseif cmd == "path" then
									local path = params
									if not step.waypath then
										step.waypath = { follow = "loose", loop = true, ants = "straight", coords = {} }
									end
									for var, val in pairs(prevpathvars) do
										step.waypath[var] = val or step.waypath[var]
									end
									...
									for coord in path:gmatch("(.-);") do
										...
										if x then
											...
											end -- might happen, if the coords look good but map is bogus.
											local point = {
												map = map or prevmap or step.map,
												floor = flr or prevfloor or step.floor,
												x = x,
												y = y,
												dist = dist or step.waypath.dist,
											}
											...
											if var:sub(1, 1) == "<" then
												...
												step.waypath.coords[#step.waypath.coords][var:sub(2)] = tonumber(val)
													or val
											else
											...
											if step.waypath.radius then
												step.waypath.dist = step.waypath.radius
											end -- radius=dist
										end
									end
								...
								elseif cmd == "only" or cmd == "if" then
									...
									if cond then
										...
										if goal.action or goal.text or chunkcount > 1 then
											if
												not ZGV:RaceClassMatch(split(params, ",")) and not ZGV.ExternalExecution
											then
												goal = {}
												break
								...
								elseif
									cmd == "more"
									and (goal.cmd == "buy" or goal.cmd == "collect" or goal.cmd == "goldcollect")
								then
								...
								elseif cmd == "noordinal" then
									goal.countord = false
								elseif cmd == "usename" then
									local name, id, obj = ParseID(params)
									goal.usename = name
									goal.useid = id
								...
								elseif GOALTYPES[cmd] and GOALTYPES[cmd].parse then
									...
									if GOALTYPES[cmd].events then
										...
										goal.eventfunc = function(...)
											GOALTYPES[cmd].onevent(goal, ...)
										end
									end
								else
									...
									local function savebrace(s, magic) -- har har indeed, quite an ugly hack to discriminate between brace types
										tinsert(braces, s)
										return not magic and ("%%BRACE" .. #braces .. "%%")
											or ("@@BRACE" .. #braces .. "??" .. magic .. "@@")
									end
									...
									local function saveway(s)
										...
										if err or not x or not y then
											...
											tinsert(
												ways,
												{
													map = map or goal.map or step.map or miniprevmap,
													floor = flr or goal.floor or step.floor or miniprevfloor,
													x = x,
													y = y,
													dist = dist,
												}
											...
											s = ""
											if not hide then
												if map ~= goal.map or flr ~= goal.floor then
													s = (ZGV.GetMapNameByID(map) or "(map " .. map .. ")") .. " "
												end
									...
									local function loadbrace(s, magic) -- for explaination what magic is see savebrace
										return not magic and ("{" .. braces[tonumber(s)] .. "}")
											or ("{=" .. braces[tonumber(s)] .. "??" .. magic .. "=}")
									end
									text = text:gsub("@@BRACE(%d+)%?%?(.-)@@", loadbrace)
										:gsub("%%%%BRACE(%d+)%%%%", loadbrace)
		...
		for si, step in ipairs(guide.steps) do
			...
			if step.extralabels then
				for _, extralabel in ipairs(step.extralabels) do
					if not steplabels[extralabel] then
						steplabels[extralabel] = {}
					end
...
function Parser:ParseHeader(guide)
	...
	if guide.headerdata.parsed then
		return
	end
	guide.headerdata.parsed = true
	...
	for cmd, params in pairs(guide.headerdata) do
		...
		if cmd == "pet" then
			...
			if not speciesId then
				return parseerror(guide.title, "bad pet number", cmd, params)
			end
			...
			local _, _, _, creatureID, _, _, _, _, _, _, _, DisplayID =
				C_PetJournal.GetPetInfoBySpeciesID(speciesId or 0)

			ZGV.CreatureDetector:RegisterPetID(creatureID, guide)
			guide.headerdata.model = DisplayID

			local cond = function()
				return haspet(speciesId)
			end

			local fun, err, cond_procd = MakeCondition(cond)
			if not fun then
				return parseerror(guide.title, err, cmd, params)
			end
		...
		elseif cmd == "sugGroup" then
			...
			local group = ZGV:FindOrCreateGroup(ZGV.registered_groups, "SUGGESTED\\" .. (folder or header))
				or ZGV.registered_groups
		...
		elseif
			cmd == "condition_suggested"
			or cmd == "condition_valid"
			or cmd == "condition_invalid"
			or cmd == "condition_end"
			or cmd == "condition_visible"
			or cmd == "condition_suggested_race"
		then
			local case
			if cmd == "condition_suggested" then
				case = "suggested"
			end
			if cmd == "condition_valid" then
				case = "valid"
			end
			if cmd == "condition_invalid" then
				case = "invalid"
			end
			if cmd == "condition_end" then
				case = "end"
			end
			if cmd == "condition_visible" then
				case = "visible"
			end
			if cmd == "condition_suggested_race" then
				case = "suggested_race"
			end
			...
			if not fun then
				return parseerror(guide.title, err, cmd, tostring(params))
			end
			guide["condition_" .. case .. "_raw"] = cond_procd or "(code)"
			guide["condition_" .. case] = fun
		elseif cmd == "condition_valid_msg" or cmd == "condition_invalid_msg" then
			guide[cmd] = apply_colours(params)
		elseif cmd == "description" then
			guide.headerdata[cmd] = apply_colours(params)
		...
		elseif cmd == "class" then
			...
			if guide.class and guide.class ~= params then
				return parseerror(
					guide.title,
					"can't specify multiple classes for a guide, make it general",
					cmd,
					params
				)
			end
			if not ParseHeader_classes[params] then
				return parseerror(guide.title, "class " .. params .. " is unknown to science", cmd, params)
			end
			--[[ unused?
		...
		elseif cmd == "spec" then
			...
			if not guide.headerdata.class then
				return parseerror(guide.title, "spec specified without a class, what do you mean?", cmd, params)
			end
			local spec = ZGV.SpecToNumber[guide.headerdata.class][params]
			if not spec then
				return parseerror(
					guide.title,
					"spec " .. params .. " unknown for class " .. tonumber(guide.class),
					cmd,
					params
				)
			end
			if not guide.spec then
				guide.spec = {}
			end
			--[[ unused?
		...
		elseif cmd == "model" and not (guide.headerdata.pet or guide.headerdata.mounts) then
			...
			for i = 1, #params do
				...
				if num then
					...
					if cached then
						...
						if file and ZGV.PET_MODEL_BASE then
							ZGV.PET_MODEL_BASE[num] = model .. ":" .. file
						end
					end
					if model then
			...
			if ZGV.CreatureDetector.PetMirror:IsShown() then
				ZGV.CreatureDetector.PetMirror:Hide()
			end -- and stay low
			guide.model = params
...
tinsert(ZGV.startups, {
	"Parser ConditionEnv:_Setup",
	function(self)
		self.Parser.ConditionEnv:_Setup()
		if ZGV.IsRetail then
			ZGV:AddEventHandler("COVENANT_CHOSEN", Parser.UpdateCovenant)
		end
	end,
	after = "Faction startup",
})
