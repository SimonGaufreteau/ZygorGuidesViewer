...
function Sync:CreatePacket_GuideStatus()
	...
	local function GetStepStatus(step)
		...
		for gi, goal in ipairs(step.goals) do
			...
			goals[gi] = (not completable and "-")
				or (complete and "c")
				or (possible and ("%d/%d"):format(numdone, numneeded))
				or "i"
		end
		return ("%d,%s;%s"):format(
			step.shared_origin or step.num,
			req and (step:IsComplete() and "c" or "i") or "x",
			table.concat(goals, ",")
		)
	end
	...
	local packet = PACKETTYPE_GUIDESTATUS
		...
		) --,ZGV.Parser.classspecs[select(2,UnitClass("player"))][GetSpecialization() or 0] or "unspec"

	return packet
end

function Sync:CreatePackets_StepData()
	if not ZGV.CurrentGuide or ZGV.CurrentGuide.headerdata.shared then
		return
	end
	...
	ret = ret
		.. ("%s##GU=%s##ST=%d##LA=%s##SL=%d\n"):format(
			PACKETTYPE_STEPDATA,
			guide_title,
			step_num,
			ZGV.CurrentStep.label or "",
			#steplines
		)
...
function Sync:OnChatReceived(msg, channel, sender)
	...
	if not msg then
		Sync:Debug("No packet received")
		return
	end

	local msg_decoded = LibDeflate:DecodeForWoWAddonChannel(msg)
	if not msg_decoded then
		Sync:Debug("No packet decoded")
		return
	end

	local msg_unpacked = LibDeflate:DecompressDeflate(msg_decoded)
	if not msg_unpacked then
		Sync:Debug("No packet unpacked")
		return
	end
	...
	for chunk in packet_iterator(msg_unpacked) do
		...
		if not packettype then
			Sync:Debug("Bad packet received: ", chunk)
			return
		end
...
function Sync:Unpack(packet, data)
	if packet.type == PACKETTYPE_GUIDESTATUS then
		...
		if packet.MS then
			packet.sharemode = tonumber(packet.MS)
		end
		if packet.CH then
...
function Sync:HandleReceivedPacket(packet)
	if packet.type ~= "nonexistent_future_packet" and not self:IsEnabled() then
		return
	end -- ignore everything else when not in Sync mood
	if packet.type == PACKETTYPE_GUIDESTATUS then
		self:Debug(
			"Player %s (%s %s level %d) is on guide %s step %d which is %s. They are %s.",
			packet.sender,
			packet.race,
			packet.class,
			packet.level,
		...
		if self:IsMaster() and packet.sharemode == 2 and (packet.is_complete or not packet.is_possible) then
			if ZGV.CurrentStepNum ~= packet.stepnum then
				if not ZGV:Throttler("SYNC_REBROADCAST", 5) then
	...
	elseif packet.type == PACKETTYPE_STEPSTICKY then
		...
		self:Debug(
			"Player %s sends sticky step %d data (%d goals needed), saving as step %d",
			packet.sender,
			packet.stepnum,
			packet.linecount,
			packet.stepnum_shared
		)
	elseif packet.type == PACKETTYPE_STEPLINE then
		if not self:IsSlave() then
		...
		if not SDB.steps then
			ZGV:Error("Sharing error: Incomplete shared guide step, data incoming without step header")
			return
		end
		local stepbuffer = SDB.steps[packet.stepnum_shared or packet.stepnum]
		if not stepbuffer then
			ZGV:Error(("Sharing error: data for step %d unexpected!"):format(self.stepnum))
			return
		end
		if stepbuffer.lines[packet.linenum] then
			ZGV:Error(("Sharing error: step %d line %d sent twice."):format(packet.stepnum, packet.linenum))
			return
		end
		stepbuffer.lines[packet.linenum] = packet.linestring
		self:Debug(
			"Player %s sends step %d line %d, saved as %d",
			packet.sender,
			packet.stepnum,
			packet.linenum,
			packet.stepnum_shared or packet.stepnum
		)
	elseif packet.type == PACKETTYPE_STEPEND then
		...
		if not SDB.steps then
			ZGV:Error("Sharing error: no step data sent before step_end")
			return
		end
		...
		if not allOK then
			ZGV:Print("Incomplete data received!")
		end
		...
		for _, step in pairs(SDB.steps) do
			if step.sticky then
				guidesource = guidesource .. "stickystart " .. step.label .. "\n"
			end
		end
		...
		self.SharedGuide = ZGV.GuideProto:New(
			"SHARED\\" .. mainstep.guide,
			{ shared = true, singlestep = mainstep.stepnum, hardcore = true },
			guidesource
		)
		self.SharedGuide:Parse(true)

		for si, step in ipairs(self.SharedGuide.steps) do
			step.is_shared = true
		end
	...
	elseif packet.type == PACKETTYPE_SLAVEREQUEST then
		if not ZGV.db.profile.sync_enabled then
			return
		end
		if ZGV.db.profile.share_masterslave == 2 then
			return
		end -- already a slave
...
function Sync:OnPartyStatusChanged()
	...
	for name, status in pairs(self.PartyStatus) do
		...
		for gi, gs in ipairs(status.goals or dummytable) do
			...
			goals = goals
				.. (
					gs.complete and "c"
					or (gs.done and ("%d/%d"):format(gs.done, gs.needed))
					or (gs.completable and "undone")
					or "-"
				)
		end
		s = s
		...
		if status.sharemode == 1 then
			self.master_present_status = status
		end
	end
	self:Debug(s)
	ZGV:UpdateFrame()
	...
	-- TODO : Controls
	-- if ZGV.Frame.Controls.GuideShareButton:IsMouseOver() then
	-- 	ZGV.Frame.GuideShareButton_OnEnter()
	-- end -- goes to Sync:OnShareButtonEnter() anyway

	-- abort slave request, if master is gone
	if self.SlaveConfirmPopup and self.SlaveConfirmPopup:IsVisible() and not self.master_present_status then
		self.SlaveConfirmPopup:Hide()
	end

	if self:IsPartyStatusComplete() then
...
function Sync:IsClearToProceed(stepnum)
	if not stepnum then
		stepnum = ZGV.CurrentStepNum
	end
	if not self:IsEnabled() or not self:IsSnapping() then
		return true
	end
	if not self.PartyStatus or not next(self.PartyStatus) then
		return true
	end
	...
	for name, status in pairs(self.PartyStatus) do
		if
			status.guide
			and (status.guide:gsub("^SHARED\\", "") == ZGV.CurrentGuide.title:gsub("^SHARED\\", ""))
			and (
				(status.stepnum < stepnum)
				or (status.stepnum == stepnum and status.is_possible and not status.is_complete)
			)
		then
			return false
		end
...
function Sync:GetStepGoalPartyStatusText(stepnum, goalnum)
	...
	local partysort = {}
	if
		IsInGroup(LE_PARTY_CATEGORY_HOME) --[[really]]
	then
		for i = 1, GetNumGroupMembers() - 1 do
			table.insert(partysort, (UnitName("party" .. i)))
		end
	else --[[fake]]
		for k, v in ZGV.OrderedPairs(self.PartyStatus) do
			table.insert(partysort, k)
		end
	end
		...
		repeat
			...
			if ZGV.db.profile.share_partydisplaystyle == 1 then
				...
				elseif goal.needed then
					if goal.done == 0 and goal.needed == 1 then
						color = 0 -- incomplete 0/1
					elseif goal.done == 0 and goal.needed == 0 then
						color = 2 -- incomplete 0/0 (impossible?)
					else
						color = 0 -- incomplete 1/3
					end
				else
					color = 2
				end
			...
			elseif ZGV.db.profile.share_partydisplaystyle == 3 then
				...
				elseif goal.done then
					if goal.done == 0 and goal.needed == 1 then
						color = "ff8888"
						display = "[ ]" -- figure space: U+2007
					elseif goal.done == 0 and goal.needed == 0 then
						color = "ff8888"
						display = "[ ]" -- figure space: U+2007
					else
						color = "ff8888"
						display = ("[%d/%d]"):format(goal.done, goal.needed)
					end
				else
					color = "888888"
					display = "[?]"
			...
			elseif ZGV.db.profile.share_partydisplaystyle == 4 then
				...
				elseif goal.needed then
					if goal.done == 0 and goal.needed == 1 then
						color = "ff8888"
						display = "" -- incomplete 0/1
					elseif goal.done == 0 and goal.needed == 0 then
						color = "ff8888"
						display = "" -- incomplete 0/0 (impossible?)
					else
						color = "ff8888"
						display = (" [%d/%d]"):format(goal.done, goal.needed) -- incomplete 1/3
					end
				else
					color = "888888"
					display = " [-]"
			...
			if status.goals[goalnum] ~= 1 then
				any_incomplete = true
			end
		until true
	end
...
function Sync:GetAheadBehind()
	if not ZGV.CurrentStepNum then
		return nil, "no step"
	end
	if self.PartyStatus and next(self.PartyStatus) then
		...
		if #ahead > 0 or #behind > 0 then
			...
			if ZGV.db.profile.share_partydisplaystyle <= 3 then
				if #ahead > 0 then
					s = "Ahead: " .. table.concat(ahead, ", ")
				end
				if #behind > 0 then
					s = s .. (#s > 0 and "; " or "") .. "Behind: " .. table.concat(behind, ", ")
				end
			elseif ZGV.db.profile.share_partydisplaystyle == 4 then
				if #ahead > 0 then
					s = table.concat(ahead, ", ")
				end
				if #behind > 0 then
					s = s .. (#s > 0 and "; " or "") .. table.concat(behind, ", ")
				end
			end
			return s
		end
...
function Sync:AnnounceStatus()
	...
	if self.fake_party then
		ZGV:ScheduleTimer(function()
			self:FakePartyGenerator()
		end, 1.0)
	end
end
...
function Sync:Init()
	...
	local function ON_ZGV_STEP_CHANGED()
		if self:IsEnabled() then
	...
	local last_change = debugprofilestop()
	ZGV:AddMessageHandler("ZGV_STEP_CHANGED", function(_, _, step)
		local current_change = debugprofilestop()
		if current_change - last_change < 1000 and not ZGV.skipping then
			if Sync.ThrottleTimer then
				ZGV:CancelTimer(Sync.ThrottleTimer)
			end
			Sync.ThrottleTimer = ZGV:ScheduleTimer(ON_ZGV_STEP_CHANGED, 1)
	...
	do
		...
		popup.OnMore = function(dialog) end
...
function Sync:UpdateButtonColor()
	if not ZGV.Frame.Controls then
		return
	end
	if not ZGV.Frame.Controls.GuideShareButton then
		return
	end
	local r, g, b, a
	if self:IsInGroup() then
	...
	ZGV.Frame.Controls.GuideShareButton
		:GetHighlightTexture()
		:SetVertexColor(r, g, b, ZGV.Frame.Controls.GuideShareButton:GetHighlightTexture():GetAlpha())
...
function Sync:ShowMasterConfirmation()
	if ZGV.PopupHandler:IsInNC("ZygorShareInitiate") then
		return
	end

	if ZGV.db.profile.sync_dontconfirm then
		return self:ActivateAsMaster()
	end

	local zygor_users = ZGV.TableKeys(self.PartyStatus)
	local notslaves = self:GetParty_NotSlaveNames()

	self.InitiateConfirmPopup:SetText(
		L["share_invite_master"]:format(
			ZGV:FormatNiceGuideTitle(ZGV.CurrentGuide.title),
			#zygor_users < GetNumGroupMembers() - 1
					and (
						#zygor_users > 1 and L["share_invite_master_party_manyzygors"]
						or L["share_invite_master_party_onezygor"]
					):format(comma_and(zygor_users))
				or ""
		)
	)
...
function Sync:GetStepSource(stepnum, fromwriter)
	...
	for line in ZGV.CurrentGuide.rawdata_full:gmatch("(.-)\n") do
		...
		if line:find("^%s*step") then
			...
			in_step = sn == stepnum
			if
				#rawstep > 0 and not in_step --[[anymore]]
			then
				break
			end
			linematch = "step"
		elseif
			line:find("^%s*stickyst[artop]*")
			or line:find("#include", 1, true)
			or line:find("leechstep", 1, true)
		then
			in_step = false
		end
		if in_step then
			table.insert(rawstep, line)
			if linematch == "step" and ZGV.CurrentGuide.steps[stepnum].map and not fromwriter then
...
function Sync:RequestAllStatuses(callback, timeout)
	...
	self.packet_callbacks[PACKETTYPE_GUIDESTATUS] = function(packet)
		...
		for i = 1, GetNumGroupMembers() - 1 do
			if not all_statuses_requested[UnitName("party" .. i)] then
				all_received = false
			end
		end
		if all_received then
			callback(all_statuses_requested)
			finalize()
		end
	end
...
function Sync:OnShareButtonEnter(button)
	...
	local notslaves = self:GetParty_NotSlaveNames()
	if not self:IsInGroup() then
		txt2 = L["share_tooltip_noparty"] --txt2=L['opt_share_enabled_desc']
	elseif #ZGV.TableKeys(self.PartyStatus) == 0 then
		txt2 = L["share_tooltip_nozygor"] --txt2=L['opt_share_enabled_desc']
	elseif self:IsSlave() then
		txt2 =
			L["share_tooltip_stopslave"]:format(self.master_present_status and self.master_present_status.sender or "?")
	elseif self:IsMaster() and #notslaves > 0 then
		txt2 = L["share_tooltip_stopmaster_or_reinvite"]
			.. "\n"
			.. L["share_tooltip_currentslaves"]:format(comma_and(self:GetParty_SlaveNames()))
	elseif self:IsMaster() and #notslaves == 0 then
		txt2 = L["share_tooltip_stopmaster"]
			.. "\n"
			.. L["share_tooltip_currentslaves"]:format(comma_and(self:GetParty_SlaveNames()))
	elseif self.master_present_status then
		txt2 = L["share_tooltip_startslave"]:format(self.master_present_status.sender)
	else
		txt2 = L["share_tooltip_startmaster"] --txt2=L['opt_share_enabled_desc']
	end
	GameTooltip:SetOwner(ZGV.Frame, "ANCHOR_TOP")
	GameTooltip:SetText(L["share_tooltip_title"])
	if txt2 then
		GameTooltip:AddLine(txt2, 0, 1, 0)
	end
	GameTooltip:Show()
end
...
function Sync:OnShareButtonClick()
	if not self:IsInGroup() then
		ZGV:Error("You are not in a group.")
		return false
	end
	...
	elseif self:IsMaster() then
		if #notslaves == 0 then
			...
			if #slaves > 0 then
				self.ReinviteOrStopPopup:SetText(
					L["share_invite_master_stop_or_reinvite_with"]:format(
						comma_and(self:GetParty_NotSlaveNames()),
						comma_and(slaves)
					)
				)
			else
				self.ReinviteOrStopPopup:SetText(
					L["share_invite_master_stop_or_reinvite"]:format(comma_and(self:GetParty_NotSlaveNames()))
				)
			end
	...
	-- any other masters?
	elseif self.master_present_status then
		self:ShowSlaveConfirmation(
			self.master_present_status.sender,
			self.master_present_status.guide,
			"slave initiated"
		)
		...
		if #ZGV.TableKeys(self.PartyStatus) == 0 then
			ZGV:Error("No-one in your party is running Zygor Guides.")
			return false
		end
		if not ZGV.CurrentGuide then
			ZGV:Error("You don't have a guide loaded.")
			return false
		end
...
tinsert(ZGV.startups, {
	...
	after = { "Parser ConditionEnv:_Setup" },
})
