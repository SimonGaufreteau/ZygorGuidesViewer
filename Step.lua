...
function Step:OnLeave()
	ZGV:Debug("&step_setup Step %d OnEnter",self.num)

	for i,goal in ipairs(self.goals) do 
		goal:UnregisterEvents()
	end
end
...
function Step:CycleWaypoint(delta,nocycle,because)
	...
	local isValid = function(goal)  return goal
		and goal.x
		and goal:IsVisible()
		and goal.action~="mapmarker"
		and not goal.force_noway
		and (not goal:IsCompleteable() or select(2,goal:IsComplete() --[[possible--]] ))  end
