...
function ItemScore:SetStatWeights(playerclass,playerspec,playerlevel)
	...
	if ZGV.IsRetail then self.playeritemlvl = GetAverageItemLevel() end
	...
	if ZGV.IsRetail or ZGV.IsClassicCATA or ZGV.IsClassicMOP then
		...
		self.active_set = ZGV.db.char.gear_selected_spec
	end
...
local function ItemScore_SetTooltipData(tooltip)
	...
	if not ItemScore.TooltipPatched then
		...
		if ZGV.db.profile.debug_display then
			...
			tooltip:AddLine("link "..(fulllink or ""))
			tooltip:AddLine("stripped "..(itemlink or ""))
			tooltip:AddLine("itemid "..(ZGV.ItemLink.GetItemID(itemlink) or ""))
		end
		...
		if valid or validfuture then
			...
			local hotkey,mousekey = "",""		
			for i,v in ipairs(ZGV.ItemScore.modifierkeys) do
				if v[1] == ZGV.db.profile.gearreportkey then hotkey = v[2] end
			end
			for i,v in ipairs(ZGV.ItemScore.mousekeys) do
