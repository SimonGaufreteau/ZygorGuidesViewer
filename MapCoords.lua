...
local force_maptype = {
	...
	[750] = Enum_Zone,
	[747] = Enum_Zone,
	[1182] = Enum_Zone, -- these zones have mapType==6, let's correct it to zone
	[626] = Enum_Micro,
	[627] = Enum_Zone,
	[628] = Enum_Micro,
	[629] = Enum_Micro, -- Dalaran L is a micro, not a dungeon - but main floor is a Zone!
	[125] = Enum_Zone,
	[126] = Enum_Micro, -- Dalaran is a micro, not a dungeon - but main floor is a Zone!
	[499] = Enum_Micro,
	[500] = Enum_Micro, -- Deeprun Tram
	...
	[1167] = Enum_Micro,
	[1166] = Enum_Micro, -- Zanchul in Dazar'alor are micro maps, not dungeon.
	--[830]=Enum_Continent,[882]=Enum_Continent,[885]=Enum_Continent, -- Argus zones
	[1349] = Enum_Micro,
	[1350] = Enum_Micro,
	[1351] = Enum_Micro, -- Tol Dagor Scenario
	...
	[720] = Enum_Micro,
	[721] = Enum_Micro, -- Fel Hammer
	[879] = Enum_Micro,
	[880] = Enum_Micro, -- Fel Hammer
	[698] = Enum_Micro,
	[699] = Enum_Micro, -- Icecrown Citadel L
	[775] = Enum_Micro,
	[776] = Enum_Micro, -- Exodar, Azuremyst (scenario)
	[804] = Enum_Micro,
	[805] = Enum_Micro, -- Scarlet Monastery
	[748] = Enum_Micro, -- Niskara
	[740] = Enum_Micro,
	[741] = Enum_Micro, -- Shadowgore Citadel
	[865] = Enum_Micro,
	[866] = Enum_Micro, -- Stormheim invasion
	[869] = Enum_Micro,
	[870] = Enum_Micro, -- Highmountain invasion
	[717] = Enum_Micro, -- Dreadscar Rift (Warlock OH)
	[1535] = Enum_Zone,
	[1534] = Enum_Zone, -- Durotar War Campaign and Orgrimmar War Campaign on Kalimdor
	[734] = Enum_Micro,
	[735] = Enum_Micro, -- Hall of the Guardian
	[791] = Enum_Micro,
	[792] = Enum_Micro, -- Temple of the Jade Serpent
	[799] = Enum_Micro,
	[800] = Enum_Micro,
	[801] = Enum_Micro,
	[802] = Enum_Micro,
	[803] = Enum_Micro, -- The Oculus scenario
	[881] = Enum_Micro,
	[872] = Enum_Micro,
	[873] = Enum_Micro,
	[874] = Enum_Micro,
	...
	[744] = Enum_Micro,
	[745] = Enum_Micro,
	[746] = Enum_Micro, -- Warrior Legion Ulduar Order Hall Scenario
	[857] = Enum_Micro,
	[829] = Enum_Micro,
	[891] = Enum_Micro,
	[892] = Enum_Micro,
	[893] = Enum_Micro,
	[894] = Enum_Micro, -- Argus Azuremyst Isle Scenario
	[889] = Enum_Micro,
	[890] = Enum_Micro, -- Arcatraz Argus Campaign
	[1681] = Enum_Micro,
	[1682] = Enum_Micro, -- Shadowlands Icecrown Citadel Intro
	[1690] = Enum_Micro, -- Aspirant's Quarters (Bastion)
	[1707] = Enum_Micro,
	[1708] = Enum_Micro, -- Elysian Hold (Bastion)
	[1670] = Enum_Zone,
	[1671] = Enum_Zone,
	[1672] = Enum_Zone,
	[1673] = Enum_Zone, -- Oribos all floors
	[1701] = Enum_Zone,
	[1702] = Enum_Zone,
	[1703] = Enum_Zone, -- Heart of the Forest (Ardenweald)
	...
	[940] = Enum_Micro,
	[941] = Enum_Micro, -- Lightforged Draenei Starter
	[1699] = Enum_Zone,
	[1700] = Enum_Zone, -- Sinfall both floors
	[2005] = Enum_Micro, -- Battle of Ardenweald scenario
	[2016] = Enum_Micro, -- Tazavesh is orphan, change to micro

	[391] = Enum_Zone,
	[393] = Enum_Zone,
	[394] = Enum_Zone, -- Pandaria Shrines should be zones
...
function ZGV.GetMapInfo(mapid)
	...
	if LibRover.data.InstanceMapsRev and LibRover.data.InstanceMapsRev[mapid] then
		ZGV_mapcache[mapid] = {
			...
			name = LibRover.data.MapGroupIDs[mapid],
			floorname = LibRover.data.MapGroupIDs[mapid],
			parentMapID = LibRover.data.InstanceMapsContinents[mapid],
		}
	else
	...
	if forced_type then
		ZGV_mapcache[mapid].mapType = forced_type
	end
...
function ZGV.GetMapChildren(mapID, resultsArray)
	...
	if mapInfo and mapInfo.children then
...
function ZGV.GetMapNameByID(id, floor)
	if type(id) == "string" then
		id, floor = LibRover:GetMapByNameFloor(id, floor)
	end
...
function ZGV.GetMapGroupID(id, floor)
	...
	if mapInfo and mapInfo.groupID == nil then
		...
		if not mapInfo.groupID then
			mapInfo.groupID = false
		end -- but not nil!
	end
...
function ZGV.GetMapFloorNameByID(id, floor)
	...
	local mapInfo = ZGV.GetMapInfo(id or 0)
	if mapInfo and not mapInfo.floorname then
		repeat
			local groupID = C_Map.GetMapGroupID(id)
			if not groupID then
				break
			end
			local groupInfo = C_Map.GetMapGroupMembersInfo(groupID)
			if not groupInfo then
				break
			end
			for i, floor in ipairs(groupInfo) do
				if floor.mapID == id then
					mapInfo.floorname = floor.name
					break
				end
			end
		until true
...
function ZGV.MapsOnDifferentFloors(m1, m2)
	...
	if cache[m1] == false then
		return false
	end
	if cache[m2] == false then
		return false
	end
...
local function GetMAPDATAFromGWP()
	for i = 1, 3000 do
		repeat
			...
			if not LibRover.data.FloorByID[i] then
				break
			end
...
local function CloneHBDMap(src, dst, force, changes)
	local srct = ZGV.HBD.mapData[src]
	if not srct then
		return
	end
	local dstt = ZGV.HBD.mapData[dst]
	if dstt and not force then
		return
	end
	dstt = {}
	ZGV.HBD.mapData[dst] = dstt
...
local function FixMAPDATA()
	...
	if MAPDATA[1535] then
		MAPDATA[1535][1] = 1
	end -- Durotar War Campaign is on Kalimdor, not on some magic instance
	if MAPDATA[1534] then
		MAPDATA[1534][1] = 1
	end -- Orgrimmar War Campaign is on Kalimdor, not on some magic instance either
	...
	if MAPDATA[1527] then
		MAPDATA[1527][1] = 1
	end -- Uldum New - also on Kalimdor
	if MAPDATA[2239] then
		MAPDATA[2239][1] = 2444
	end -- Amirdrassil on Dragon Isles
	--if MAPDATA[2133] then MAPDATA[2133][1]=2444 end -- Zaralek on Dragon Isles, but disabled, caverns and overworld don't mix well

	if MAPDATA[2070] then
		MAPDATA[2070][1] = 0
	end -- Tirisfal L is on EK
...
function MapCoords.Mdist(map1, x1, y1, map2, x2, y2)
	...
	if not dm1 or not x1 or not x2 then
		return nil, nil, nil, "err, no start"
	end
	if map1 == map2 then -- same map
		...
		if not dm2 then
			return nil, nil, nil, "err, dest unknown"
		end
		if dm1[1] ~= dm2[1] then
			return nil, nil, nil, "err, diff continent", dm1[1], dm2[1]
		end -- diff continent
...
function MapCoords.Mxlt(map1, x, y, map2, oob_ok, creative)
	...
	if not map1 or not map2 then
		return false, nil, nil, "no maps"
	end

	if
		map2 == MAP_AZEROTH --[[AZEROTH--]]
	then
		if map1 == MAP_COSMIC then
			return false, nil, nil, "cosmic <-> azeroth"
		end
	...
	elseif
		map2 == MAP_COSMIC --[[COSMIC--]]
	then
		if map1 == MAP_AZEROTH then
			return false, nil, nil, "cosmic <-> azeroth"
		end
		local x, y, zone = ZGV.HBD:GetWorldCoordinatesFromZone(x, y, map1)
	...
	elseif XM12 then
		...
		if not oob_ok and not overlap then
			return false, nil, "oob"
		end
	else
		local dm1 = MAPDATA[map1]
		local dm2 = MAPDATA[map2]
		if not dm1 or not dm2 then
			return false
		end
		if dm1[1] ~= dm2[1] then -- diff continent
			if creative then
				...
				if not dm1 then
					return nil, nil, false, "creative fail"
				end
				was_creative = true
			else
				XM1[map2] = false
	...
	if (x2 < 0 or x2 > 1 or y2 < 0 or y2 > 1) and not oob_ok then
		return false, nil, nil, "not oob"
	end
...
function MapCoords.TranslateVirtualContinents()
	for parent, pdata in pairs(virtual_continents) do
		local _, par_cx, par_cy, par_cw, par_ch = unpack(MAPDATA[continent_main_maps[parent]]) -- TODO : Currently errors, do we need this ?
...
function MapCoords.Mxlt_get_virtual_cont_data(map_new, map_ori)
	...
	D.virt_new_posx, D.virt_new_posy, D.virt_new_wid, D.virt_new_hei =
		virt_new_posx, virt_new_posy, virt_new_wid, virt_new_hei
...
function MapCoords.Mxlt_test()
	...
	ZGV:ScheduleTimer(function()
		...
		for i = 1, 1000000 do
			HBD:TranslateZoneCoordinates(0.2, 0.2, 85, 65, false)
		end
		local t2 = debugprofilestop()
		print(("%.3fms"):format(t2 - t1))
	end, 0.3)
	...
	function MapCanvasScrollControllerMixin:CreateZoomLevels()
		...
		for layerIndex, layerInfo in ipairs(layers) do
			...
			for zoomLevelIndex = 0, numZoomLevels - 1 do
				currentScale =
					math.max(layerInfo.minScale * zoomDeltaPerStep ^ zoomLevelIndex, currentScale + MIN_SCALE_DELTA) --sinus
...
function MapCoords:SetupMapButtons()
	...
	if MapCoords.MapButtonFrame then
		return
	end
	...
	MapCoords.MapButtonFrame =
		ZGV.ChainCall(ZGV.CreateFrameWithBG("FRAME", "ZygorWorldMapMenu", WorldMapFrame.BorderFrame))
			...
			:SetScript("OnUpdate", function(self)
				if MapCoords.MapButton:IsDragging() then -- yes, it's the inside button that's dragging the parent.
					...
					self:ClearAllPoints()
					local dx, dy =
						min(max(mx - px + 10, mapbuttonradius), parent:GetWidth() + mapbuttonradius),
						min(max(my - py - 10, -mapbuttonradius), parent:GetHeight() - mapbuttonradius)
	...
	MapCoords.MapButton = ZGV
		.ChainCall(CreateFrame("BUTTON", nil, MapCoords.MapButtonFrame, "ZygorSpecialButton_Template"))
		:ApplySkin()
		:SetSize(32, 32)
		...
		:SetScript("OnClick", function()
			MapCoords:ShowMapMenu()
		end)
		:SetScript("OnDragStart", function(self)
			self:GetParent():SetMovable(true)
			self:GetParent():StartMoving()
		end) -- yes, this frame's drag initiates its parent's drag.
...
function MapCoords:ShowMapMenu()
	...
	if ZGV.db.profile.poienabled then
		tinsert(menu, {
			...
			tooltipOnButton = 1,
			func = function()
				ZGV:SetOption("Maps", "poienabled off")
				ZGV.Poi:ChangeState(false)
			end,
			notCheckable = 0,
		})
		local poiTypeList = {}
		for i = 1, #POI_TYPES do
			...
			tinsert(poiTypeList, {
				...
				checked = function()
					return not ZGV.db.profile.hideguide[keyword]
				end,
				func = function()
			...
			menuList = {
				{
					...
					keepShownOnClick = true,
					checked = function()
						return (ZGV.db.profile.poitype == 1)
					end,
					func = function()
				...
				},
				{
					...
					keepShownOnClick = true,
					checked = function()
						return (ZGV.db.profile.poitype == 2)
					end,
					func = function()
		...
		tinsert(menu, {
			...
			tooltipOnButton = 1,
			func = function()
				ZGV:SetOption("Maps", "poienabled on")
				ZGV.Poi:ChangeState(true)
			end,
			notCheckable = 0,
		})
	...
	tinsert(menu, {
		...
		checked = function()
			return ZGV.db.profile.worldquestenable
		end,
		func = function()
