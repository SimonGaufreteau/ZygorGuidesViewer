...
ZGV.StandAlone = nil -- C_AddOns.GetAddOnMetadata(addonName,"X-Standalone") TODO : Check if this is needed
-- local GetAuraDataByIndex = C_UnitAuras.GetAuraDataByIndex TODO : Not used
...
ZGV.HBD = LibStub("HereBeDragonsQuestie-2.0")
ZGV.HBDPins = LibStub("HereBeDragonsQuestie-Pins-2.0")
...
ZGV.IsClassicSoM = C_Seasons
	and (ZGV.IsClassic and C_Seasons.HasActiveSeason() and C_Seasons.GetActiveSeason() == Enum.SeasonID.SeasonOfMastery)
ZGV.IsClassicSoD = C_Seasons
	and (
		ZGV.IsClassic
		and C_Seasons.HasActiveSeason()
		and C_Seasons.GetActiveSeason() == (Enum.SeasonID.SeasonOfDiscovery or 2)
	)
ZGV.IsClassicHardcore = C_Seasons
	and (
		ZGV.IsClassic
		and C_Seasons.HasActiveSeason()
		and (
			C_Seasons.GetActiveSeason() == Enum.SeasonID.Hardcore
			or C_Seasons.GetActiveSeason() == Enum.SeasonID.FreshHardcore
		)
	)
ZGV.IsClassicAnniv = C_Seasons
	and (
		ZGV.IsClassic
		and C_Seasons.HasActiveSeason()
		and (
...
ZGV.IsClassicAnnivHardcore = C_Seasons
	and (ZGV.IsClassic and C_Seasons.HasActiveSeason() and C_Seasons.GetActiveSeason() == Enum.SeasonID.FreshHardcore)

-- remix gets rechecked during OnEnable since PGTSID does not return proper values on load
ZGV.IsPandariaRemix = PlayerGetTimerunningSeasonID
	and PlayerGetTimerunningSeasonID() == Constants.TimerunningConsts.TIMERUNNING_SEASON_PANDARIA
...
if ZGV.IsRetail then
	ZGV.IMAGESDIR = ZGV.DIR .. "\\Guides-Retail\\Images\\"
end
if ZGV.IsClassicTBC then
	ZGV.IMAGESDIR = ZGV.DIR .. "\\Guides-TBC\\Images\\"
end
if ZGV.IsClassicWOTLK then
	ZGV.IMAGESDIR = ZGV.DIR .. "\\Guides-WOTLK\\Images\\"
end
if ZGV.IsClassicCATA then
	ZGV.IMAGESDIR = ZGV.DIR .. "\\Guides-CATA\\Images\\"
end
if ZGV.IsClassic then
	ZGV.IMAGESDIR = ZGV.DIR .. "\\Guides-Classic\\Images\\"
end
if ZGV.IsClassicMOP then
	ZGV.IMAGESDIR = ZGV.DIR .. "\\Guides-MOP\\Images\\"
end
...
do
	...
	ZGV.Expansion_Mists = false
	ZGV.Expansion_Warlords = false
	ZGV.Expansion_Legion = false
	ZGV.Expansion_Shadowlands = false
...
local BZR = setmetatable({ _table = BZ:GetReverseLookupTable() }, {
	__index = function(t, k)
		return t._table[k] or k
	end,
})
ZGV.BZL = BZL
ZGV.BZR = BZR
...
function ZGV:OnInitialize() --ADDON_LOADED
	...
	self.startuptimes["Loading guides"] = ZGV.startuptimestamps["guides/autoload.xml end"]
		- ZGV.startuptimestamps["guides/autoload.xml start"]
	self.startuptimes["Loading code"] = ZGV.startuptimestamps["files.xml end"]
		- ZGV.startuptimestamps["files.xml start"]
		- self.startuptimes["Loading guides"]
	...
	-- Ensure your mixin table exists
	ZygorGuidesViewerMapIcon_Mixin = ZygorGuidesViewerMapIcon_Mixin or {}

	-- Grab the frame
	local icon = ZygorGuidesViewerMapIcon

	-- Apply mixin
	Mixin(icon, ZygorGuidesViewerMapIcon_Mixin)

	-- Now Setup should exist
	if icon.Setup then
		icon:Setup()
	end
	...
	if IsShiftKeyDown() then
		...
		for k, v in pairs(self.db.char) do
			if k:match("^maint_") then
				self.db.char[k] = false
			end
		end
	...
	if not self.db.char.guides_history_ZGV4clear then
		self.db.char.guides_history = {}
		self.db.char.guides_history_ZGV4clear = true
	end
	if not tonumber((next(self.db.char.guides_history))) then -- convert back to flat
		local g = {}
		for gtype, guide in pairs(self.db.char.guides_history) do
			tinsert(g, guide)
		end
	...
	if ZGV.DEV and ZGV.LibTaxi and ZGV.LibTaxi.errors and next(ZGV.LibTaxi.errors) then
		ZGV:Print("DEV: LibTaxi reports errors. See LibTaxi.errors")
	end
	...
	if ZGV.IsRetail then
		if not ZGV.Expansion_Shadowlands then
			hooksecurefunc("SendQuestChoiceResponse", function(...)
				ZGV:QuestRewardSelect(...)
			end)
		end
		hooksecurefunc(C_PlayerChoice, "SendPlayerChoiceResponse", function(...)
			ZGV:PlayerChoiceResponce(...)
		end)
	end

	if self.DEV then
		ZGV.DebugFrame =
			ZGV.ChainCall(CreateFrame("FRAME", "ZygorDebugFrame", UIParent)):SetPoint("TOPLEFT"):SetSize(1, 1).__END
		ZGV.DebugFrame.text1 =
			ZGV.ChainCall(ZGV.DebugFrame:CreateFontString()):SetPoint("TOPLEFT"):SetFontObject(SystemFont_Tiny).__END
	...
	if self.ERRORS then
		self:Print("Errors were detected on startup, see ZGV.ERRORS")
	end
	...
	for k, v in pairs(_G) do
		if k:find("___ZGV_TIMESTAMP", 1, true) then
			_G[k] = nil
		end
	end
...
function ZGV:OnEnable() --PLAYER_LOGIN
	...
	self:AddEventHandler("UPDATE_MOUSEOVER_UNIT", function()
		ZGV.HandleRaidmarker("mouseover")
	end)
	self:AddEventHandler("UNIT_TARGET", function()
		ZGV.HandleRaidmarker("target")
	end)
	...
	local function register_for_suggestion(pin)
		if pin:GetScript("OnMouseUp") then
			hooksecurefunc(pin, "OnMouseUp", function(pin, button)
				ZGV:SuggestGuideFromBlizzardIcon(pin)
			end)
		else
			pin:EnableMouse(true)
			pin:SetScript("OnMouseUp", function(pin)
				ZGV:SuggestGuideFromBlizzardIcon(pin)
			end)
		end
	end
	...
	for pinmixin, pintemplate in pairs(mixin_to_template) do
		...
		if mixin then
			...
			for pin, _ in WorldMapFrame:EnumeratePinsByTemplate(pintemplate) do
				register_for_suggestion(pin)
			end
		end
	end
...
function ZGV:Startup_LoadGuides_Threaded()
	...
	if #self.registeredguides > 0 then
		...
		for i, guide in ipairs(self.registeredguides) do
			repeat
				...
				end -- may yield, caught by @coro_startup
				...
				if debugprofilestop() - t > (self.LOADGUIDES_INTENSITY or self.STARTUP_INTENSITY) then -- let it take us down to 3fps, it's the startup. -- NOOOO! - 2016-03-28 21:21:06 sinus
					...
					yield(
						"loadguides: "
							.. (full_load and "parsing fully" or "parsing headers")
							.. " ("
							.. guides_this_tick
							.. ")",
	...
	self.RegisterGuide = function()
		ZGV:Print("Too late to RegisterGuide at this point!")
	end
...
local function waitformaint(maint)
	while not ZGV.db.char[maint] do
		yield("waiting for " .. maint)
	end
...
function ZGV:StartupModule_Threaded(startup, timeleft) -- resumed in _StartupThread. Returns when module is 100% done. May yield, or may let the module startup yield.
	...
	startup.thread = coroutine.create(function()
		startup.func(self)
	end)

	if coroutine.status(startup.thread) ~= "suspended" then
		self:ErrorThrow("Error during initialization sequence: module '" .. startup.name .. "' didn't start")
	end
	...
	--local gc=0
	repeat
		if self.db.profile.safe_startup then
			t = debugprofilestop()
			yield("Before startup module: " .. startup.name .. " ...")
		end
		local t1 = debugprofilestop()
		...
		if not ok then
			self:ErrorThrow(
				"Error during initialization sequence '"
					.. startup.name
					.. "':\n"
					.. tostring(ret)
					.. "\n"
					...
					.. ZGV.version
					.. ", WoW v"
					.. table.concat({ table.removemulti({ GetBuildInfo() }, 1, 2) }, ".")
					.. "\n"
			)
		end
		---
		...
		if self.db.profile.safe_startup then
			t = debugprofilestop()
			yield(("Startup module: |cffffddee%s|r took %d ms"):format(startup.name, t1))
		end
		...
		if debugprofilestop() - t > timeleft then
			t = debugprofilestop()
			timeleft = yield("(startup module: " .. startup.name .. ")", progress, msg) or timeleft
		end -- caught by @coro_startup

	until coroutine.status(startup.thread) == "dead"

	self:Debug(
		"&startup Starting module: |cffddffaa%s|r done in |cffffddee%d|rms.",
		startup.name,
		debugprofilestop() - t0
	)
end
...
local function _StartupThread()
	...
	if self.Pointer then
		self.Pointer:Startup()
	end
	if self.Foglight then
		self.Foglight:Startup()
	end
	...
	if ZGV.db.profile.debug and ChatFrame1 and ChatFrame1.SetMaxLines then
		ChatFrame1:SetMaxLines(2000)
	end
	...
	for i, startup in ipairs(self.startups) do
		if type(startup) == "table" then
			startup.name, startup.func = unpack(startup)
		elseif type(startup) == "function" then
			self.startups[i] = { name = "unnamed(" .. i .. ")", func = startup }
			startup = self.startups[i]
		end
		if type(startup.after) == "string" then
			startup.after = { startup.after }
		end
	end
	local function startup_is_done(s)
		for i, startup in ipairs(self.startups) do
			if startup.name == s or startup.id == s then
				return startup.done
			end
		end
	end
		...
		for i, startup in ipairs(self.startups) do
			if not startup.done then
				if not startup.after or all_done(startup.after) then
					...
					self:Debug(
						"&startup Module |cffddffaa%s|r |cffffee88awaits|r |cffddffaa%s|r",
						startup.name,
						table.concat(startup.after, ",")
					)
				end
			end
		end
	...
	local function run_startup_phase(phase)
		for cycles = 1, 20 do
			...
			if done == 0 then
				yield("Nothing done, ending cycles")
				break
			end
		end
	end
	...
	for i, startup in ipairs(self.startups) do
		if not startup.done then
			error("Startup: " .. startup.name .. " still not done!!!")
		end
	end
	...
	self:Debug(
		"&startup Loading time - total: %.2f",
		self.startuptimes["Loading libs"] + self.startuptimes["Loading guides"] + self.startuptimes["Loading code"]
	)
...
function ZGV.startuptimes:View()
	local ret = {}
	for i, t in ipairs(self.index) do
		tinsert(ret, ("%s = %.3f ms"):format(t, self[t]))
	end
	return ret
end
...
function ZGV.startuptimestamps:View()
	...
	for i, v in ipairs(ret) do
		ret2[i] = ("%d (+%d): %s"):format(v[2], (i > 1) and (v[2] - ret[i - 1][2]) or 0, v[1])
	end
	return ret2
end
...
function ZGV:StartupStep() -- called from MasterFrame
	if not last_gettime then
		last_gettime = GetTime()
	end
	if last_gettime == GetTime() then
		return
	end -- ah-ha, NOT loaded then!
	if ZGV.IsRetail then
		if UnitExists("player") and UnitInVehicle("player") then
			self.loading_screen_disabled = true
		end -- Blizz... if we are in vehicle, loading_screen_disabled will never fire after loading
	end

	if UnitExists("player") then
		self.loading_screen_disabled = true
	end
	if not self.loading_screen_disabled then
		return
	end
	...
	if self.db.profile.debug_detailedstartup then
		self:Debug("&startup Startup frame %d...", startup_frames)
	end

	if GetFramerate() < 40 then
		ZGV.STARTUP_INTENSITY = 5
	elseif GetFramerate() < 60 then
		ZGV.STARTUP_INTENSITY = 10
	else
		ZGV.STARTUP_INTENSITY = 20
	end
	if ZGV.db.profile.loadguidesfully then
		ZGV.STARTUP_INTENSITY = 100
	end -- loadfully is allowed to be slow
	if ZGV.db.profile.loadguidesfully and ZGV.loadprogress and ZGV.loadprogress - lastprogress > 0.05 then
		ZGV:Print(("Loading... %d%%"):format(ZGV.loadprogress * 100))
		lastprogress = ZGV.loadprogress
	end

	while debugprofilestop() - thisframet < self.STARTUP_INTENSITY do
		local t = debugprofilestop()

		local good, msg, progress, msg2 =
			resume(thread, max(0, self.STARTUP_INTENSITY - debugprofilestop() + thisframet)) -- THIS IS WHERE STARTUP THINGS HAPPEN. yield: msg,progress,msg2  -- @coro_startup
		...
		if self.db.profile.debug_detailedstartup or lastret ~= msg then
			self:Debug(
				"&startup &_SUB0 Startup frame %d tick %d |cffeeff88%s|r (%s%d%%) took |cffffeeaa%d|rms",
				startup_frames,
				startup_ticks,
				tostring(msg),
				msg2 and msg2 .. "; " or "",
		...
		if GetTime() - lastrettime > STARTUP_SPAM_FREQUENCY then
			lastret = nil
		end
		...
		if not good then
			...
			error(
				"ERROR in startup frame " .. startup_frames .. ": " .. tostring(msg) .. "\nin\n" .. debugstack(thread),
				2
			)
		...
		elseif status(thread) == "dead" then
			...
			self:Debug(
				"&startup Startup complete in %.2f (%d ticks in %d frames)",
				startup_time,
				startup_ticks,
				startup_frames
			)
...
function ZGV:AddEventHandler(event, handler)
	...
	if #self.Events_Events[event] == 1 then
		self:RegisterEvent(event, "EventHandler_Events")
	end
	return handler
end
...
function ZGV:AddMessageHandler(event, handler)
	...
	if #self.Events_Messages[event] == 1 then
		self:RegisterMessage(event, "EventHandler_Messages")
	end
	return handler
end
...
function UpdateCentral_Mixin:AddHandler(handler)
	if type(handler) ~= "function" then
		error("Update handler " .. tostring(handler) .. " is not a function")
	end
...
function UpdateCentral_Mixin:AddHiding(object)
	if type(object) ~= "table" then
		error("Hiding handler " .. tostring(object) .. " is not a frame")
	end
...
function UpdateCentral_Mixin:RemoveHiding(removeobject)
	if type(removeobject) ~= "table" then
		error("Hiding handler " .. tostring(removeobject) .. " is not a frame")
	end
...
function UpdateCentral_Mixin:OnEvent(event)
	if event == "GLOBAL_MOUSE_UP" then -- special handler for GMU
		for i, object in ipairs(self.HideObjects) do
			if object and object:IsVisible() and not object:IsMouseOver() then
				object:Hide()
			end
		end
	end
...
function ZGV:LoadInitialGuide(fastload)
	...
	if ZGV.db.char.maint_startup_startguide then
		if self.db.char.guidename and string.find(self.db.char.guidename, "GOLD\\Crafting\\") then
			...
			ZGV:Debug(
				"&startup Loading initial guide: %s step %d",
				self.db.char.guidename or "?",
				self.db.char.step or 0
			)
		...
		elseif not self.CurrentGuide and not fastload then
			if self.db.char.tabguides and next(self.db.char.tabguides) then -- try loading one of existing tabs first
				...
				if not gs or #gs == 0 then
					self.Frame:SetSpecialState("select")
					self:Print(
						"No guides suggested for your char. Please open guide menu and select the guide you want to use."
					)
				elseif #gs == 1 then
					self:SetGuide(gs[1])
				else --many
...
ZGV.GuideTitles = {
	["SUGGESTED"] = L["guidepicker_suggested"],
	["RECENT"] = L["guidepicker_recent"],
	["SEARCH"] = L["guidepicker_searchresults"],
...
function ZGV.StepHistory:Prune()
	...
	for i, guide in ipairs(to_remove) do
		_gsh[guide] = nil
	end
end
...
function ZGV:SetGuide(name, step, source, silent, prevguide)
	...
	elseif type(name) == "string" then
		name = name
			:gsub("^Zygor's Alliance ", "")
			:gsub("^Zygor's Horde ", "") -- code-side fix for "common" guides.
			:gsub("^Zygor's ", "")
	...
	if guide then
		if prevguide ~= nil then
			if not ZGV.GuideFuncs:IsValid(guide, step, "rating", nil, nil, prevguide) then
				return
			end
		else
			if not ZGV.GuideFuncs:IsValid(guide, step, source or "setguide") then
				return
			end
		end
		...
		if guide.steps then
			...
			if guide.headerdata.singlestep then
				step = guide.headerdata.singlestep
			end
...
function ZGV:FindSuggestedGuides(typ)
	...
	in ipairs(scope) do
		...
		if guide.sugGroup and (status == "VALID" or status == "SUGGESTED") then
			if not suggroups[guide.sugGroup] then
				suggroups[guide.sugGroup] = {}
			end
...
function ZGV:ForeachInGuidesAsync2(guides, callback, progress, done)
	self.Promise
		:New()
		:Defer(function(success, failure)
			local d1 = debugprofilestop()
			for i, guide in ipairs(guides) do
				callback(guide)
				if i % 10 == 0 then
					if debugprofilestop() - d1 > 0.05 then
						if progress then
							progress(i)
						end
						yield()
...
function ZGV:GetGuideFolderInfo(folder)
	...
	for i, guide in ipairs(folder.guides) do
		if guide.GetStatus and guide:GetStatus() == "SUGGESTED" then
			suggest = true
		end
	end
	if not suggest then
		for i, group in ipairs(folder.groups) do
			if self:GetGuideFolderInfo(group) then
				suggest = true
			end
		end
	end
...
function ZGV:FocusStep(num, forcefocus)
	...
	if type(num) == "string" and self.CurrentGuide.steplabels then
		...
		self:Debug("&step FocusStep: %s = %s", s, tostring(num))
	end
	...
	if num > #self.CurrentGuide.steps then
		return
	end
	if self.CurrentGuide.headerdata.singlestep then
		num = self.CurrentGuide.headerdata.singlestep
	end
	...
	local reportlabel = self.CurrentStep
			and ZGV.QuestDB.GetStepTag
			and (ZGV.QuestDB:GetStepTag(self.CurrentStep) or "") .. " "
		or ""
	reportlabel = reportlabel
		.. ("(From %s step %s, fast forward %s, skipping %s"):format(
			tostring(prevguide),
			tostring(prevnum),
			tostring(self.fastforward),
			tostring(self.skipping)
		)
	...
	if prevFocus or forcefocus then
		ZGV:SetStepFocus(self.CurrentStep)
	end
	...
	if (cs ~= self.CurrentStep) or (cg ~= self.CurrentGuide) then
		self:Debug("&step FocusStep: guide or step changed! bailing.")
		return
	end
	...
	for i, goal in ipairs(self.CurrentStep.goals) do
		...
		if goal.zombiewalk then
			self.CurrentStep.zombiewalk = true
		end
	end
	...
	if self.pause then
		if
			(self.db.profile.skipimpossible and not steppossible)
	...
	if cs.map then
		cs.map = self.Pointer:SanitizePhase(cs.map)
	end
	for gi, goal in ipairs(cs.goals) do
		if goal.map then
			goal.map = self.Pointer:SanitizePhase(goal.map)
		end
	end
	...
	if
		ZGV.db.profile.preview
		and (not GetPlayerFacing() or ZGV.db.char.fakeinstance)
		and ZGV.db.profile.preview_control == "step"
	then
...
function ZGV:TrackQuest(id)
	if tonumber(GetCVar("autoQuestWatch")) == 0 then
		return
	end
	if ZGV.IsClassicCATA then
		return
	end -- SetSuperTrackedQuestID on cata crashes game

	local q = ZGV.questsbyid[id]
	if not q or not q.inlog then
		return
	end
	if ZGV.IsRetail then
...
function ZGV:PreviousStep(fast, forcefocus)
	...
	if self.completionstreak == 0 then
		ZGV.completioninterval = ZGV.completionintervallong
	end -- first skip
	...
	if not self.StepHistory:HasHistory(guidename) then
		...
		self.pause = not fast
		if fast then
			self:SendMessage("ZGV_STARTED_SKIPPING")
		end
	end
...
function ZGV:SkipStep(fast, hack, forcefocus) --Hack used for testing, forces showing endguide popup
	...
	if self.completionstreak then
		ZGV.completioninterval = ZGV.completionintervallong
	end -- first skip
	if ZGV.Sync:IsSlave() then
		return
	end -- don't skip
	...
	if self.CurrentStep.needsreload then
		return self:ReloadStep(fast)
	end

	if not nextstep and not hack then -- when not forced, that is: usually.
		local nextstep2, stepnum, guide

		if fast and self.db.profile.instantskip then
			nextstep2, stepnum, guide = self.CurrentStep:GetNextCompletableStep()
			self:Debug("Next completable step is: %d", stepnum)
		else
			nextstep2, stepnum, guide = self.CurrentStep:GetNextValidStep()
			if nextstep2 then
				self:Debug("Next valid step is: %d", nextstep2.num)
			elseif guide then
				self:Debug("Next valid step is: %d in %s", stepnum, guide.title)
			else
				self:Debug("Next valid step: none!")
			end
	...
	if (not nextstep or hack) and not self.CurrentGuide.headerdata.poiloader then
		-- final step
		self.pause = true
		self.fastforward = false
		if self.skipping then
			self:SendMessage("ZGV_STOPPED_SKIPPING")
		end
		self.skipping = false
		local nextguide
...
function ZGV:TryToCompleteStep(force)
	...
	if not self.CurrentStep or not self.CurrentGuide then
		return
	end

	-- frame hidden? bail.
	if not self.Frame:IsVisible() or self.Frame:GetAlpha() < 0.1 then
		return
	end
	--
	...
	if
		(not self.CurrentStep:AreRequirementsMet() or (self.db.profile.skipimpossible and not steppossible))
	...
	if self.CurrentStep.ispoiloader then
		completing = false
	end
	...
	for i, goal in ipairs(step.goals) do
		if goal:IsCompleteable() then
			...
			if self.recentGoalProgress[goal] == nil then
				self.recentGoalProgress[goal] = done
			end
			if
				self.recentGoalProgress[goal] ~= done
				and (goal.action ~= "goto" or (self.recentGoalProgress[goal] == 1 ~= done == 1))
			then -- announce and animate anything BUT a goto.
				goal.dirtytext = true
				goalsneedanimating[goal] = true
				self:SendMessage("ZGV_GOAL_PROGRESS", step.num, i)
				self:Debug(
					"Goal progress: step %d goal %d progress %d->%d",
					step.num,
					i,
					self.recentGoalProgress[goal] or -1,
					done or -1
				)
			end
			self.recentGoalProgress[goal] = done
	...
	if confirmfound and confirmcompleted ~= true then
		completing = false
	end
	...
	if self.pause or self.db.profile.dontprogress then
		...
		if completing then
			...
			self:Debug(
				"Skipping step: %d (%s)",
				self.CurrentStepNum,
				(stepcomplete and "complete" or (steppossible and "possible?" or "impossible"))
			)
			local s = ""
			for gn, goal in ipairs(self.CurrentStep.goals) do
				s = s .. goal:GetDebugDump() .. "\n"
			end
			...
			end -- first skip, set speed to slow and begin speeding up.

			self.fastforward = true

			interval = interval * self.completionintervalspeed
			if interval < self.completionintervalmin then
				interval = self.completionintervalmin
			end
	...
	ZGV.Tabs:CheckForStepCompletion()
...
function ZGV:MaybeSuggestNextGuide()
	...
	if self.CurrentGuide.condition_suggested_exclusive then
		return
	end
	if self.CurrentStepNum == #self.CurrentGuide.steps then
		return
	...
	if
		not nextguidetitle
		or ZGV.db.char.ignoredguides[nextguidetitle]
		or (ZGV.tempguideblock and ZGV.tempguideblock[nextguidetitle])
	then
		return
	end -- boo.
...
function ZGV:UpdateFrame(full, onupdate)
	do
		return self:DoUpdateFrame(full, onupdate)
	end -- bandaid bypass
	self.updateframe_dirty = true
	self.updateframe_dirty_full = full
	self.updateframe_dirty_onupdate = onupdate
	if not self.updateframetimer then
		self.updateframetimer = self:ScheduleRepeatingTimer("UpdateFrame_Schedule", 0.01)
	end
end
...
function ZGV:DoUpdateFrame(full, onupdate)
	...
	if
		ZGV.BugReport.GuideRating.GuideRatingViewer
		and ZGV.BugReport.GuideRating.GuideRatingViewer:IsVisible()
		and (ZGV.Frame.Border:GetHeight() < 274)
		and (not ZGV.BugReport.GuideRating.ZygorPopup or not ZGV.BugReport.GuideRating.ZygorPopup:IsVisible())
	then
		ZGV.BugReport.GuideRating:ShowGuideRating()
	end

	if
		ZGV.BugReport.GuideRating.ZygorPopup
		and ZGV.BugReport.GuideRating.ZygorPopup:IsVisible()
		and ZGV.Frame.Border:GetHeight() >= 274
	then
		ZGV.BugReport.GuideRating:ShowGuideRating()
	...
	if self.CurrentGuide and self.CurrentGuide.steps and self.CurrentGuide.fully_parsed then
		...
		local stickies
		if
			self.db.profile.stickyon
			and (self.db.profile.stickydisplay == 3 or self.db.profile.stickydisplay == 4)
			and not showallsteps
		then
		...
		for stepnum = firststep + 1, laststep do
			ZGV.Frame:AddStep(self.CurrentGuide.steps[stepnum])
		end
...
function ZGV:ResizeFrame(source)
	...
	local function CalculateHeight()
		...
		if self.Frame.Controls.DefaultStateButton:IsShown() then
			return self.Frame.Controls.DefaultStateButton.min_height
		end
		for i, stepframe in ipairs(self.Frame.stepframes) do
			if stepframe:IsShown() then
	...
	if self.Frame.SetResizeBounds then
		self.Frame:SetResizeBounds(
			MIN_WIDTH,
			CONTROLS_HEIGHT + (
					autoresize and 0 --[[n/a]]
					or MIN_SCROLLABLE_CONTENT_HEIGHT
				),
			MAX_WIDTH,
			CONTROLS_HEIGHT + MAX_CONTENT_HEIGHT
		)
	else
	...
	if autoresize then
		if ZGV.CurrentGuide and ZGV.CurrentStep.score then
			...
			elseif
				ZGV.BugReport.GuideRating.CancelledRatingFrame
				and ZGV.BugReport.GuideRating.CancelledRatingFrame:IsVisible()
			then
				adjusted_contentheight = 115
			elseif
				ZGV.BugReport.GuideRating.GuideRatingViewer and ZGV.BugReport.GuideRating.GuideRatingViewer:IsVisible()
			then
				adjusted_contentheight = 244
			end
		else
		...
		if adjusted_contentheight ~= nil then
			local height = max(adjusted_contentheight + CONTROLS_HEIGHT, MIN_HEIGHT)
			self.Frame:SetHeight(height)
		end
	else
		-- do we need the scrollbar?
		local scrollbar_needed = allstepsmode
			or contentheight > Scroll:GetHeight() + 0.1
			or contentheight > MAX_CONTENT_HEIGHT
...
function ZGV:ScrollToCurrentStep() -- deprecated: no more scrolling
	...
	if self.CurrentStep and self.db.profile.displaymode == "guide" then
		...
		if self.db.profile.showcountsteps == 0 then
			...
			if
				topstep > self.CurrentStepNum --above
...
function ZGV:RaceClassMatch(fit, nocache, r, c)
	...
	if not nocache and RaceClassMatchCache[fitstring] ~= nil then
		return RaceClassMatchCache[fitstring]
	end

	if type(fit) == "table" then
		for i, v in ipairs(fit) do
			local r, x = self:RaceClassMatch(v)
			if r then
				return r, x
			end
		end
	...
	if not nocache and RaceClassMatchCache[fit] ~= nil then
		return RaceClassMatchCache[fit]
	end
	...
	fit = fit:gsub("UNDEAD", "SCOURGE")
		:gsub("LFDRAENEI", "LIGHTFORGEDDRAENEI")
		:gsub("HMTAUREN", "HIGHMOUNTAINTAUREN")
		:gsub("ZTROLL", "ZANDALARITROLL")
...
function ZGV:RaceClassMatchList(list)
	...
	for fit in list:gmatch("(.-),") do
		if self:RaceClassMatch(fit) then
			return true
		end
	end
end
...
function ZGV:Print(s, ifdebug, force)
	...
	if not force and not ZGV.db.profile.noisy then
		return
	end
...
function ZGV:ThunderStageForceUpdate()
	do
		return
	end -- TODO: reimplement this with C_Map
	if thunder_stack then
		return
	end
...
function ZGV:GetThunderStageQQ()
	...
	if GetCurrentMapAreaID() ~= 928 then
		return ZGV.db.char.thunderstage
	end -- the best we can do, if we weren't forcing it
...
function ZGV:FindEvent(eventName)
	...
	for event = 1, numEvents do
		...
		if
			eventdata
			and eventdata.calendarType == "HOLIDAY" --We don't care about any other events
			and (eventName == ZGV.WorldEventIDs[eventdata.eventID] or eventName == eventdata.title)
		then --Does the ring fit?
...
StartupTimingFrame:SetScript("OnEvent", function(f, event, arg)
	if event == "ADDON_LOADED" and arg == addonName then
		ZGV.startuptimestamps:Punch("addonloaded")
	end
	if event == "VARIABLES_LOADED" then
		ZGV.startuptimestamps:Punch("variablesloaded")
	end
end)
...
function ZGV:SetStepFocus(step)
	if step.isFocused or (step:IsCurrentlySticky() and ZGV.CurrentStep.isFocused) then
		return
	end -- no change.
...
function ZGV:IsStepFocused(step)
	if step.isFocused then
		return true
	end
	if ZGV.CurrentStep.isFocused and step:IsCurrentlySticky() then
		return true
	end
	return false
end
...
function ZGV:GoalOnClick(frame, button)
	...
	if curTime - self.GoalClickedTime < 0.2 then
		return
	end
	self.GoalClickedTime = curTime
	local goalframe = frame
	if goalframe.parentLine then
		goalframe = goalframe.parentLine
	end
	...
	if self.db.profile.showcountsteps > 0 and stepframe.step ~= self.CurrentStep and not stepframe.is_sticky then
		return
	...
	if button == "LeftButton" and not goal.parentStep.isFocused then
		return
	end -- don't act if we are not focused
...
function ZGV:GoalOnEnter(goalframe)
	...
	if goal.x and goal.y and goal.map then
		...
		local tipformat = (self.db.profile.windowlocked or goal.force_noway) and "tooltip_waypoint_coords"
			or "tooltip_waypoint"
		local coords = math.round(goal.x * 100) .. ";" .. math.round(goal.y * 100)
		local map = (ZGV.GetMapNameByID(goal.map) or ("#" .. goal.map)) .. " "
		wayline =
			L[tipformat]:format(goal.waytitle and (goal.waytitle .. " (" .. map .. coords .. ")") or map .. coords)
	end
	...
	if infoline or wayline or image or goal.itemid or tooltipline or hyperlink then
		...
		local lines = 1
		if infoline then
			GameTooltip:AddLine(infoline, 1, 1, 1)
			lines = lines + 1
		end
		if wayline then
			GameTooltip:AddLine(wayline, 1, 1, 1)
			lines = lines + 1
		end
		if tooltipline then
			GameTooltip:AddLine(tooltipline, 1, 1, 1)
			lines = lines + 1
		end

		for line = 1, lines do
			if _G["GameTooltipTextLeft" .. line]:GetWidth() > 300 then
				_G["GameTooltipTextLeft" .. line]:SetWidth(300)
			end
		end
...
function ZGV:GoalOnLeave(goalframe, num)
	if goalframe and (GameTooltip:GetOwner() == goalframe or GameTooltip:GetOwner() == self.Frame) then
		GameTooltip:Hide()
	end
end
...
local function insert_guides(arr, guides)
	...
	for i, guide in ipairs(guides) do
		...
		local item = {
			...
			func = function()
				CloseDropDownForks()
				ZGV:SetGuide(guide.full, guide.step)
			end,
...
function ZGV:OpenGuideMenu(path)
	if self.Menu then
		if
			path == "HOME"
			and ZGV.CurrentGuide
			and not ZGV.CurrentGuideName:match("GOLD\\")
			and not ZGV.CurrentGuideName:match("PETSMOUNTS\\Pets")
		then
			ZGV.GuideMenu:Show()
...
function ZGV:FakeCompleteGoal(goal, docomplete)
	if docomplete == nil then
		docomplete = not self.recentlyCompletedGoals[goal]
	end
	if docomplete then
		--self.recentlyCompletedGoals[goal]=true
...
function ZGV:FakeCompleteQuest(questid, docomplete, questtitle)
	...
	if questid then
		self.completedQuests[questid] = docomplete
	end
	self:Print(
		"Marking quest '"
			.. questtitle
			...
			.. " as "
			.. (docomplete and "completed" or "incomplete")
	)
	self:TryToCompleteStep(true)
end
...
function ZGV:OpenQuickStepMenu(stepframe, goalframe)
	...
	if goal.map and goal.x then
		...
		tinsert(menu, {
			...
			tooltipOnButton = true,
			func = function()
				goal.parentStep:CycleWaypointTo(goal.num)
			end,
			notCheckable = true,
			indented = true,
		})
	...
	if goal:IsCompleteable() then
		tinsert(menu, {
			...
			func = function()
				self:FakeCompleteGoal(goal, not self.recentlyCompletedGoals[goal])
			end,
			isNotRadio = true,
		})
		if goal.quest then
			...
			if inlog then
				if ZGV.IsRetail then
					tinsert(menu, {
						...
						tooltipOnButton = true,
						func = function()
							QuestMapFrame_OpenToQuestDetails(goal.quest.id)
						end,
					})
					tinsert(menu, {
						...
						tooltipOnButton = true,
						checked = function()
							return C_QuestLog.GetQuestWatchType(goal.quest.id)
						end,
						func = function()
				...
				if quest_map and quest_map > 0 then
					tinsert(menu, {
						...
						tooltipOnButton = true,
						func = function()
							OpenWorldMap(quest_map)
							ZGV:PointToQuest(quest_map, goal.quest.id)
						end,
					})
				end
			end
			...
			if self.db.profile.debug_display then
				tinsert(menu, {
					...
					tooltipTitle = self.completedQuests[goal.questid] and L["qmenu_quest_complete_tip"]
						or L["qmenu_quest_complete_in_tip"],
					tooltipText = self.completedQuests[goal.questid] and L["qmenu_quest_complete_desc"]
						or L["qmenu_quest_complete_in_desc"],
					checked = function()
						return self.completedQuests[goal.questid]
					end,
					func = function()
						self:FakeCompleteQuest(goal.questid, not self.completedQuests[goal.questid], goal.quest.title)
					end,
					isNotRadio = true,
				})
				tinsert(menu, {
					...
					checked = function()
						return self.DEBUG_QUEST_ID == goal.questid
					end,
					func = function()
						if self.DEBUG_QUEST_ID == goal.questid then
							self.DEBUG_QUEST_ID = nil
							self:Debug("Not tracing.")
						else
							self.DEBUG_QUEST_ID = goal.questid
							self:Debug("Tracing quest %d", goal.questid)
						end
					end,
					isNotRadio = true,
				})
	...
	tinsert(menu, {
		...
		menuList = {
			{
				text = L["qmenu_shareto_party"],
				checked = function()
					return self.db.profile.share_target == "PARTY"
				end,
				func = function()
					self.db.profile.share_target = "PARTY"
					CloseDropDownForks()
				end,
			},
			{
				text = L["qmenu_shareto_raid"],
				checked = function()
					return self.db.profile.share_target == "RAID"
				end,
				func = function()
					self.db.profile.share_target = "RAID"
					CloseDropDownForks()
				end,
			},
			{
				text = L["qmenu_shareto_say"],
				checked = function()
					return self.db.profile.share_target == "SAY"
				end,
				func = function()
					self.db.profile.share_target = "SAY"
					CloseDropDownForks()
				end,
			},
		},
	...
	if rolegoals then
		tinsert(menu, {
			...
			func = function()
				step:ShareToChat(self.db.profile.share_target or "SAY", "rolegoals", "brand")
			end,
		})
	end
...
function ZGV:OpenMapToQuestGoal(questid, goalnum) end
...
function ZGV:RegisterGuide(title, header, data)
	...
	if ZGV.BETAguides and guide then
		guide.beta = true
	end
	if not guide then
		return
	...
	if not self.registered_guide_types[guide.type] then
		self.registered_guide_types[guide.type] = {}
	end
	tinsert(self.registered_guide_types[guide.type], guide)
end

local placeholder_header_data = {}
local placeholder_get_status = function()
	return "MISSING", ""
end
...
function ZGV:UpdateMapButton()
	if self.db.profile.showmapbutton then
		ZygorGuidesViewerMapIcon:Show()
	else
		ZygorGuidesViewerMapIcon:Hide()
	end
...
function ZGV:ProfilerReport()
	...
	if GetCVar("scriptProfile") ~= "1" and not self.ProfilerMode then
		self:Print(
			"Profiling is not enabled, cannot do this, sorry. Please enable the profiler in Zygor settings and rety."
		)
		return
	end
	...
	if self.ProfilerRunning then
		...
		for fun, tab in pairs(culprits) do
			...
			if tab.count > 0 then -- We don't want THIS much litter, do we?
				table.insert(self.db.profile.profiler_stats, {
					name = tab.name,
					fulltime = tab.fulltime,
					puretime = tab.puretime,
					count = tab.count,
					fullavg = tab.fulltime / tab.count,
					pureavg = tab.puretime / tab.count,
				})
			end
		end
		...
		table.sort(self.db.profile.profiler_stats, function(n1, n2)
			return n1.fulltime > n2.fulltime
		end)
...
function ZGV:Debug(msg, ...)
	...
	table.wipe(display)
	for i = 1, select("#", ...) do
		display[i] = select(i, ...) or "nil"
	end
	-- just in case:
	table.insert(display, 0)
	table.insert(display, 0)
	table.insert(display, 0)

	local depth = 0
	if profile.debug_showdepth then
		debugstack():gsub("\n", function()
			depth = depth + 1
		end)
	end
	...
	while msg:sub(1, 1) == "&" do
		...
		if flag then
			...
			elseif flag == "_REPLACE" then
				...
				if flagdata == false then
					return
				end -- Any flag false = message is out.  "false" strictly, otherwise assume it SET!
				if type(flagdata) == "table" then
					if not flagdata.enabled then
						return
					end
					if flagdata.color then
	...
	if stack_delayed_change then
		ZGV.DEBUG_DEPTH = ZGV.DEBUG_DEPTH + stack_delayed_change
	end
	...
	if replace_id then
		msg = msg .. REPLACECODE .. "[" .. replace_id .. "]|r"
	end
	...
	if true then -- self and self.db and self.db.profile and self.db.profile.debug and not self.db.profile.quiet then
		...
		local message
		if func then
			message = ("|cffffee77Z|r: %s%06.03f+%03d|r |cff00ddbb#%d:|r %s%s  |cffaaaaaa(%s)"):format(
				timecolor,
				(t - self.timestamp_loaded_GT),
				debugms,
				self.DebugI,
				debugcolor,
				formatted_msg,
		...
		elseif self.db.profile.debug_fps then
			message = ("|cffffee77Z|r: %s%06.03f+%03d|r@%03d |cff00ddbb#%d:|r %s%s"):format(
				timecolor,
				(t - self.timestamp_loaded_GT),
				debugms,
				GetFramerate(),
			...
			message = ("|cffffee77Z|r: %s%06.03f+%03d|r |cff00ddbb#%d:|r %s%s"):format(
				timecolor,
				(t - self.timestamp_loaded_GT),
				debugms,
				self.DebugI,
		...
		if replace_id then -- find same message in history
			...
			repeat
				...
				if msg.message:find(REPLACECODE .. "[" .. replace_id, 1, true) then
					replace_at = i
					break
				end
				i = i - 1
...
function ZGV:Debug_FlagsMenu()
	...
	for k, v in pairs(ZGV.db.profile.debug_flags) do
		ZGV.Debug_FlagsTable[setmetatable({}, {
			__tostring = function()
				return k .. ": " .. (ZGV.db.profile.debug_flags[k].enabled and "ON" or "OFF")
			end,
		})] = function()
			ZGV:SetOption("Cover", "debugflag " .. k)
		end
	end
	...
	for i = from, to do
		...
		if level then
			...
			qlog = qlog
				...
				.. ": "
				.. tostring(title)
				.. (type(nobjs) == "table" and " {" .. table.concat(nobjs, ",") .. "}" or "")
				.. "|n"
		end
	end
	if Chatter then
...
-- encraption.
function ZGV:CraptOnReload(name) end
...
function ZGV:SetMagicKey(reset)
	...
	if
		not ZGV.db.profile.magickey
		or ZGV.db.profile.magickey == ""
		or InCombatLockdown()
		or not ZGV.CurrentStep
		or not ZGV.CurrentStep.goals
		or not ZGV.Frame:IsShown()
	then
	...
	if
		self.db.profile.magickey_targetcorpse
		and self.MagicKey.retarget_time
		and (GetTime() - self.MagicKey.retarget_time < 3)
		and not UnitName("target")
	then
	...
	for gi, g in ipairs(self.CurrentStep.goals) do
		...
		elseif
			targetid
			and ((UnitName("target") == g.target) or (targetid == (g.npcid or (g.mobs and g.mobs[1] and g.mobs[1].id))))
		then
...
function ZGV:MagicRaidMarker(marker)
	if not ZGV.db.profile.targetonclick then
		return ZGV
	end
...
function ZGV:MacroClickGoal(stepnum, goalnum)
	if self.CurrentGuide and self.CurrentGuide.steps[stepnum] and self.CurrentGuide.steps[stepnum].goals[goalnum] then
		self.CurrentGuide.steps[stepnum].goals[goalnum]:OnClick()
	end
end
...
function ZGV:Unparse(id, y, m, d)
	...
	local s2 = ""
	for di, i in ipairs({ id / 100000, id / 1000, id / 10, id, y, m, d }) do
		s2 = s2 .. enc(b(math.floor(i) % 10))
	end
	return s2
end
...
function ZGV:GetFlatListOfGroups(source, search, target, notoplevel, curlevel)
	...
	if not source.groups then
		return
	end
	for gi, g in pairs(source.groups) do
		if
			not (notoplevel and curlevel == 1)
			and (
				not search
				or g.name
					and (g.name:lower():find(search, 1, true) or g.name:lower():gsub("%p", ""):find(search, 1, true))
			)
		then
			tinsert(flatlist, g)
		end
		if #g.groups > 0 then
			self:GetFlatListOfGroups(g, search, target, notoplevel, curlevel + 1)
		end
	end
	return flatlist
end
		...
		for gi, g in pairs(scope) do
			if not (g.type == "TEST") and not (logic == "OR" and already_found[g]) then
				if g.title_short and g.title_short:lower():find(sub, 1, true) then
					if
						not (
							ZGV.db.profile.gmhidecompleted
							and (g:GetStatus() == "OUTLEVELED" or g:GetStatus() == "COMPLETE")
						)
					then
						tinsert(into, g)
...
function ZGV:FindFilteredGuides(filters)
	...
	if not FindFilteredGuidesCache[filterstring] then
		...
		for _, guide in ipairs(ZGV.registeredguides) do
			...
			if valid then
				table.insert(FindFilteredGuidesCache[filterstring], { guide.title, guide.title_short })
			end
		end
	end
...
function ZGV:LoadGuidesByType(guidetype)
	assert(guidetype)
	if guideTypesLoaded[guidetype] then
		return
	end
...
function ZGV:LoadNeededGuides()
	...
	for gi, g in pairs(self.registeredguides) do
		if g.need_to_parse then
			...
			g.need_to_parse = nil
			if debugprofilestop() - t0 > 100 then
				return nil
			end -- 0.1s allowed here
		end
	end
	return true
end
...
function ZGV:StartFPSFrame()
	if not ZGV.FPSFrame then
		local SIZE = 300
		local MAXFPS = 60
		ZGV.FPSFrame = ZGV.ChainCall(CreateFrame("FRAME", "ZygorFPSFrame", UIParent))
			:SetPoint("BOTTOMLEFT")
			:SetSize(SIZE + 11, MAXFPS)
			:SetFrameStrata("DIALOG").__END
		...
		for b = 1, SIZE do
			tinsert(
				ZGV.FPSFrame.bars,
				ZGV.ChainCall(ZGV.FPSFrame:CreateTexture())
					:SetPoint("BOTTOMLEFT", ZGV.FPSFrame, "BOTTOMLEFT", b, 0)
					:SetSize(1, MAXFPS)
					:SetColorTexture(1, 1, 1)
					:SetDrawLayer("ARTWORK", 1).__END
			)
			tinsert(
				ZGV.FPSFrame.fpsbars,
				ZGV.ChainCall(ZGV.FPSFrame:CreateTexture())
					...
					:SetSize(1, 2)
					:SetColorTexture(1, 1, 1)
					:SetDrawLayer("ARTWORK", 2).__END
			)
		end
		ZGV.FPSFrame.fpsbar = ZGV.ChainCall(ZGV.FPSFrame:CreateTexture())
			:SetPoint("BOTTOMLEFT", ZGV.FPSFrame.bars[SIZE], "BOTTOMRIGHT", 3, 0)
			:SetSize(5, MAXFPS)
			:SetColorTexture(1, 1, 1)
			:SetDrawLayer("ARTWORK", 1).__END
		ZGV.FPSFrame.hicbar = ZGV.ChainCall(ZGV.FPSFrame:CreateTexture())
			:SetPoint("BOTTOMLEFT", ZGV.FPSFrame.bars[SIZE], "BOTTOMRIGHT", 10, 0)
			:SetSize(5, MAXFPS)
			:SetColorTexture(1, 1, 1)
			:SetDrawLayer("ARTWORK", 1).__END
		ZGV.FPSFrame.value = ZGV.ChainCall(ZGV.FPSFrame:CreateFontString())
			:SetPoint("TOPLEFT", 5, -5)
			:SetFont("Fonts\\ARIALN.TTF", 14, "OUTLINE").__END
		...
		function ZGV.FPSFrame:OnUpdate(elapsed)
			...
			for b = 1, SIZE + 1 do
				...
				if b <= SIZE then
					self.fpsbars[b]:SetPoint("BOTTOMLEFT", b, fpslineheights[b])
				end
			end
...
function ZGV:Hook_QuestChoice()
	if ZGV.Expansion_Shadowlands then
		return
	end
	if not ZGV.IsRetail then
		return
	...
	hooksecurefunc("SendQuestChoiceResponse", function(...)
		ZGV.Surrogate_SendQuestChoiceResponse(...)
	end)
...
function ZGV.PLAYER_CHOICE_UPDATE()
	...
	for i = 1, choice.numOptions do
		...
		playerchoices[choice.choiceID][i] = playerchoices[choice.choiceID][i]
			or { selected = false, name = (c.header or "") .. " " .. (c.subheader or "") }
...
function ZGV:PlayerChoiceCleanUp()
	if not ZGV.db.char.playerchoices then
		return
	end
...
function ZGV:QuestRewardSelect(choice)
	...
	for i, v in pairs(questrewards) do
		if v.quest == id then
			questrewards[i] = nil
		end -- unset previous choices for this quest
	end
...
function ZGV:QuestRewardConfirm()
	local questrewards = ZGV.db.char.questrewards
	for i, v in pairs(questrewards) do
		if v.unconfirmed then
			v.unconfirmed = nil
		end -- mark previous unconfirmed choice as confirmed
	end
end

function ZGV:WarnAboutDebugSettings()
	if self.db.profile.fakecombat then
		self:Print("WARNING, DEBUG: Faked combat is on.", nil, true)
	end
	if self.db.profile.fakeskills and next(self.db.profile.fakeskills) then
		self:Print("WARNING, DEBUG: Fake professions are set.", nil, true)
	end
	if self.db.profile.fakereps and next(self.db.profile.fakereps) then
		self:Print("WARNING, DEBUG: Fake reputations are set.", nil, true)
	end
	if self.db.profile.fakelevel then
		self:Print("WARNING, DEBUG: Fake level is set to " .. self.db.profile.fakelevel, nil, true)
	end
	if self.db.profile.debug_librover_fakenofly then
		self:Print("WARNING, DEBUG: Fake No Flying is enabled", nil, true)
	end
end

local legion_popup_class = {
	"Warrior",
	"Paladin",
	"Hunter",
	"Rogue",
...
function ZGV:PLAYER_LEVEL_UP(event, level)
	...
	if not ZGV.IsRetail then
		if ZGV.db.profile.n_popup_skills and ZGV.db.profile.n_popup_skills_level then -- use payload level, unitlevel is not yet updated
			ZGV:ScheduleTimer(function()
				ZGV.Skills:ShowSkillPopup(level)
			end, 5)
		end
	else
		--[[
...
function ZGV:SuggestGuideFromBlizzardIcon(object)
	...
	if not guide or not guide.steplabels then
		return
	end --Clickable PoI that are not quest related, like PvP towers in Outland
	...
	if not step_label and poiID then
		for guidetitle, guidedata in pairs(guidetitles) do
			...
			if not guide.parsed then
				guide:Parse(true)
			end -- possible FPS hit!
...
function Checklist:CatchEvent(event, ...)
	if not self.events_fired[event] then
		...
		local s = event
			.. " f="
			.. self.framenum
			.. (" t=%.3f"):format(debugprofilestop() - self.starttime)
			.. "  DG "
			.. ((abs((ZGV.HBD:TranslateZoneCoordinates(0.5, 0.5, 1077, 0, 1018, 0) or 0) - 0.41) < 0.1) and "OK" or "FAIL")
			.. "  TSL "
			.. ((abs((ZGV.HBD:TranslateZoneCoordinates(0.5, 0.5, 1072, 0, 1024, 0) or 0) - 0.347) < 0.1) and "OK" or "FAIL")
			.. "  you're in "
			.. ZGV.GetMapNameByID(C_Map.GetBestMapForUnit("player") or 0)
		--print(s)
		tinsert(self.events_sequence, s)
	end
	if
		self.events_fired["_FIRST_FRAME_"]
		and self.events_fired["QUEST_LOG_UPDATE"]
		and self.events_fired["_GUIDES_LOADED_"]
		and self.events_fired["ZONE_CHANGED_NEW_AREA"]
	then
...
function ZGV.GetGarrisonLevel(real)
	...
	if not real and (ZGV.db.profile.fake_garrison or -1) > -1 then
		return ZGV.db.profile.fake_garrison
	end
...
local function getmaptexture(map)
	...
	if textures[1] then
		return textures[1].fileDataIDs[1]
	end
end
...
function ZGV.InPhase(phasename)
	...
	if phasename == ZGV.db.profile.fakephase then
		return true
	end
	...
	elseif phasename == "undercityooze" then
		return (quest(51443) or quest(60361) or quest(46727) or quest(58983))
			and not (quest(65655) or quest(65656))
			and not quest(52758) -- Started BFA, not done 9.2.5 quests, no timetravel
...
function ZGV:TestPhases()
	local phases = {
		...
		"newdustwallow",
		"oldpeak",
		"warfrontarathiassault",
		"warfrontarathicontrol",
		"warfrontdarkshoreassault",
		"warfrontdarkshorecontrol",
	}
...
function ZGV.IsLegionBoatLock()
	return (IsQuestFlaggedCompleted(40519) or IsQuestFlaggedCompleted(43926))
		and not (IsQuestFlaggedCompleted(40593) or IsQuestFlaggedCompleted(40607))
end
...
function ZGV:SetBeta(val)
	...
	if self.db.profile.debug_beta ~= nil then
		ZGV.BETA = self.db.profile.debug_beta
		return
	end
...
function ZGV:IsDragonshrineUp(name)
	...
	if not ZGV.DragonShrineCache then
		ZGV.DragonShrineCache = {}
	end
	if C_Map.GetBestMapForUnit("player") ~= 897 then
		return ZGV.DragonShrineCount
	end
...
function ZGV:IsBoosted(boost, do_popup)
	...
	if type(boosted) == "boolean" then
		return boosted
	end
	if boost == "any" or boost == "at all" then
		return ZGV.db.char.isboosted[90] or ZGV.db.char.isboosted[100] or ZGV.db.char.isboosted[110]
	end
	if not do_popup then
		return nil
	end

	if ZGV.PopupHandler:IsInNC("ZygorBoostConfirm") then
		return nil
	end
	...
	if not self.BoostConfirmPopup then
		...
		popup.OnAccept = function(self)
			...
			if ZGV.db.char.isboosted[110] then
				ZGV.db.char.isboosted[100] = true
			end
			if ZGV.db.char.isboosted[100] then
				ZGV.db.char.isboosted[90] = true
			end
		end
...
function ZGV.HandleRaidmarker(unit)
	...
	if not ZGV.db.profile.mouseovermarkers then
		return
	end
	...
	if UnitGUID("pet") == UnitGUID(unit) then
		return
	end -- don't flag own pets
...
function ZGV.ClearRaidmarker()
	...
	if ZGV.UsedRaidMarkers[1] then
		ZGV.UsedRaidMarkers[1] = nil
		SetRaidTarget("player", 1)
	end
	if ZGV.UsedRaidMarkers[6] then
		ZGV.UsedRaidMarkers[6] = nil
		SetRaidTarget("player", 6)
	end
	if ZGV.UsedRaidMarkers[7] then
		ZGV.UsedRaidMarkers[7] = nil
		SetRaidTarget("player", 7)
	end
	if ZGV.UsedRaidMarkers[8] then
		ZGV.UsedRaidMarkers[8] = nil
