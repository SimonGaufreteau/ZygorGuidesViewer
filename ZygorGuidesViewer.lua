...
ZGV.IsClassicHardcore =  C_Seasons and (ZGV.IsClassic and C_Seasons.HasActiveSeason() and (C_Seasons.GetActiveSeason()==Enum.SeasonID.Hardcore or C_Seasons.GetActiveSeason()==Enum.SeasonID.FreshHardcore))
...
function ZGV:OnEnable()  --PLAYER_LOGIN
	...
	local function register_for_suggestion(pin)
		if pin:GetScript("OnMouseUp") then
			hooksecurefunc(pin,"OnMouseUp", function(pin,button) ZGV:SuggestGuideFromBlizzardIcon(pin) end)
		else
			pin:EnableMouse(true)
			pin:SetScript("OnMouseUp",function(pin) ZGV:SuggestGuideFromBlizzardIcon(pin) end)
		end
	end
...
function ZGV:StartupStep()  -- called from MasterFrame
	...
	if ZGV.db.profile.loadguidesfully then ZGV.STARTUP_INTENSITY=100 end -- loadfully is allowed to be slow
	if ZGV.db.profile.loadguidesfully and ZGV.loadprogress and ZGV.loadprogress-lastprogress>0.05 then ZGV:Print(("Loading... %d%%"):format(ZGV.loadprogress*100))  lastprogress=ZGV.loadprogress  end
...
function ZGV:SkipStep(fast,hack,forcefocus) --Hack used for testing, forces showing endguide popup
	...
	if ZGV.Sync:IsSlave() then return end -- don't skip
